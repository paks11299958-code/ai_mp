...
> 강지훈 전용 진입화면과 `ebook` 보드 연결은 `doc/features/persona_entry_jihoon.md`를 참고한다.
...
- **2026-07-26(AI 표지 정식화 + 큐 안정화 + 책 주제/제목 분리, 사장 실사용 지적 연속)**:
  1. **표지 프롬프트 반복 개선**: 클로드→제미나이 프롬프트 엔진 교체(3자 비교: 클로드만 클리셰 규칙 무시 확인), 우주·유령 클리셰 금지, 로고 금지, 한 장에만 나오는 소재를 주인공 삼지 않기, 아동용 톤 금지, 제목 크기 상한(세로 30%·가로 80%) 명시. 색상/서체를 매번 AI가 책 톤에 맞춰 직접 고르게(무채색·원색파랑 배제).
  2. **제미나이+GPT 2안 동시생성으로 전환**(500P): `generateEbookCoverGpt` 신설(gpt-image-2, high, 1024×1536). 두 엔진 병렬 호출, 한쪽 실패해도 나머지 반환. `EbookCoverSlot` 타입 신설.
  3. **★비동기 큐+폴링 전환(핵심 버그 수정)**: GPT high가 ~95초 걸려 Vercel 프록시 타임아웃(502)에 걸림 — 서버는 성공 저장, 브라우저는 502 수신하는 실사고 재현·확인(로그로 저장 시각 대조). 그림 자리와 동일 패턴(등록만 즉시 202, `ebookImageQueue.ts` 백그라운드 틱 처리, `GET /:id/cover-queue-status` 5초 폴링)으로 전환. `EbookProject.coverCandidatesJson` 컬럼 신설(raw SQL). 이어서 "표지 큐가 그림 자리 큐에 무한정 밀리는" 우선순위 버그 발견·수정(틱마다 먼저 볼 큐를 교대).
  4. **제목을 이미지에 직접 삽입 — 여러 방식 실험 후 최종 확정**: (a) 고정 템플릿으로 제목 문자열을 프롬프트 맨 앞에 강제 삽입 → 성공률 개선했으나 완전하지 않음 (b) 서버 측 폰트 합성(sharp 설치 + 무료 한글 OFL 폰트 10종 다운로드, `lib/ebookCoverText.ts` 신설 — SVG+data URI 폰트 임베딩으로 배경 위에 텍스트 합성) → 실제 검증 성공(오탈자 0%)했으나 "AI가 통째로 그린 게 더 예쁘다"는 사장 판단으로 **git revert, 폰트 자산 전부 삭제**. (c) 최종: "원제를 참고해 AI가 더 멋진 카피로 자유롭게 재창작"하도록 프롬프트 재작성(핵심 키워드만 유지, 저자명·가짜 부제 창작 금지 가드).
  5. **책 주제/책 제목 필드 분리**(사장 지시: "표지 선택하면 그 안의 제목이 책 제목에 들어가야 한다"): 기존 제목 입력칸을 "책 주제"(topic, 자유 편집)로 전환, 별도 읽기전용 "책 제목"(title, 목차·표지가 실제 참고하는 확정값) 신설. `PUT /:id/toc`가 topic도 받도록 확장. `extractCoverTitleText(imageUrl)` 신설(제미나이 비전으로 표지 이미지 속 카피를 재확인) — 표지 선택 시 `saveCoverUrl(url, extractTitle:true)` 호출, 추출 성공 시 제목 수정모드가 열리고 draft가 채워지되 **자동 확정하지 않음**, 사용자가 확인·수정 후 저장해야 title이 바뀜(사장 지시: "확인후 수정할수 있게, 그럼 틀릴 염려가 없자나").
  6. **목차 다시 만들기**: `POST /:id/regenerate-toc` 신설 — 현재 저장된 title을 주제 삼아 목차(chapters)만 재생성, title 자체는 덮어쓰지 않음. "목차 수정" 버튼 옆에 배치, confirm() 대신 기존 `pointConfirm` 디자인 모달 재사용(icon 옵션화).
  7. **UI 배치**: 표지 만들기 섹션을 탭5(초안 만들기)→탭1(제목·목차) 제목 카드 바로 아래로 이동. 확대보기 라이트박스(`zoomImage` state)+이미지 저장 기능 신설(`saveCoverImage`를 URL 인자 받는 범용 함수로 일반화). 버튼·미리보기가 한 줄에 다닥다닥 붙어 있던 배치를 줄바꿈으로 정리(버튼 줄 / 미리보기+저장·제거 줄 분리).
  8. **★프론트 state 미초기화 버그**: "책 지우고 새 주제로 만들었더니 예전 표지 후보가 그대로 보인다" — DB엔 없는데 브라우저 state(`coverCandidates` 등)만 남아있던 현상. `handleCreate`가 `setSelected`만 하고 탭5 관련 state를 안 비웠던 게 원인. `selected?.id` 변경 시 `coverCandidates`/`coverMaking`/`imgPrompts`/`imgGenResults`/`imgGenFailed`/`imgGenBusy`/`imgGenProgress`를 전부 초기화하도록 기존 useEffect에 추가.
  9. **★서버 배포 절차 교훈 → 자동 배포 신설**: work_index.md에 "서버1 자동 git pull+PM2 reload"라고 적혀 있었으나 실제로는 동작하지 않아 88분간 서버1이 구버전으로 방치, 프론트만 새 응답 형식이라 사용자 화면에 502 실오류 발생. 원인을 파고드니 **crontab·git hooks·배포 스크립트가 전부 없어 그런 자동화는 존재한 적이 없었음**(고장이 아니라 문서에만 있던 유령 기능). → 자동 배포를 실제로 구축: `shared-api/scripts/autodeploy.sh`(서버1 사본 `~/shared-api-autodeploy.sh`) + crontab `* * * * *`. 1분마다 origin/main과 로컬 HEAD 비교해 다를 때만 pull+reload(평소 fetch 1회, 배포 약 25초). 안전장치: flock 중복실행 방지 / 로컬 미커밋 변경 시 중단 / package.json·schema.prisma 변경 시에만 npm install·prisma generate / **`tsc --noEmit` 통과해야만 reload** / 실패 시 reload 안 하고 `~/shared-api-autodeploy.log`에 사유 기록. 실전 검증 완료(푸시 후 무개입 1분 내 자동 반영·online 확인). ※API 응답 형식이 바뀌는 변경은 여전히 **백엔드 먼저 푸시 → 1분 대기 → 프론트** 순서를 지킬 것(자동배포도 1분 주기).
  배포: shared-api(초반엔 수동 git pull+pm2 reload, ⑨ 이후로는 푸시만 하면 자동 반영. DB: `coverCandidatesJson` 컬럼 추가)·ai_mp(master 다수 회 push, Vercel Promote to Production 필요).
  10. **표지 프롬프트 후속 보강**(같은 날 별도 세션, shared-api `f4033ef`): ChatGPT 앱 제작 표지와 비교하다 "노트북 화면 속 UI/코드 잔글씨가 깨진 문자로 렌더링"되는 문제 확인 — 규칙 2건 추가: (a) 화면 속 UI/코드는 읽히는 실제 문장을 만들지 말고 색상 블록·창 틀·흐릿한 라인 패턴으로만 암시(이미지 모델은 잔글씨를 정확히 못 그림) (b) 입문서·실용서는 인물+작업환경 구도(책상·노트북·스탠드 조명·화분·머그컵 소품) 우선 고려, 단 매번 같은 구도 반복 금지. `generateEbookCoverPrompt` 산출물을 제미나이/GPT 두 엔진이 공유하므로 2안 모두 적용. ※같은 세션 실측: 기존 이미지를 비슷하게 재현하려면 text-to-image 재생성(65~70%)이 아니라 원본 입력 이미지 편집(92~95%)을 써야 함 — 전역 스킬 `image-edit-sync` 참고.
