...
## 📄 대본 가져오기 + 장면별 화자 (2026-08-22, 사장 지시)

사장이 다른 도구(팟캐스트형 기획안 등)에서 **이미 완성한 대본**을 그대로 태우는 경로.
발단은 사장의 6장면짜리 부부상담 대본이었다 — 화자 2명(지은 DJ·민호 전문가),
장면별 내레이션·자막·화면 연출이 전부 적힌 완성품.

### 왜 필요한가

기존 흐름은 `주제 한 줄 → AI가 시나리오 5개 → 골라서 대본 확정`이다. **완성된 대본을
가진 사람에게는 이 3단계가 전부 방해물이다** — 후보 5개 중에 내 것과 같은 게 없다.
쇼츠 기능이 "AI가 알아서" 도구라서, 직접 쓰는 사람에게는 AI가 장애물이 되는 구조였다.

### ① 장면별 화자 (`make_short.py`)

`make_short()`는 **원래부터 세그먼트마다 `tts()`를 따로** 부르고 있었고 `tts()`도
`voice_name` 인자를 이미 받고 있었다. 그런데 영상 전체 공용 값 하나를 매번 똑같이
넘겨서 화자를 나눌 수가 없었다. 실질 수정은 한 줄이다.

```python
seg_voice = str(seg.get("voiceName") or "").strip() or voice_name
tts(seg["text"], mp3, jwt, language, category, seg_voice)
```

기존 대본에는 `seg.voiceName`이 없으므로 **동작이 한 글자도 안 바뀐다**(폴백 7종 실측).
대담·인터뷰·상담 형식 전반에 쓰인다.

### ② 대본 파싱 (`shorts_maker_worker.py`)

`parse_user_script()` — 대본 텍스트를 `build_script_draft`와 **같은 스키마**로 바꾼다.
같은 스키마여야 뒤쪽(`render_scene_images` → `make_short`)을 한 줄도 안 고치고 재사용한다.

★**LLM에게 다시 쓰게 하지 않는다. 추출·정규화만 시킨다.**
재작성을 허용하면 공들여 쓴 대본이 AI 말투로 뭉개진다. `temperature=0`(기존 생성
경로는 0.8~0.9 — 목적이 창작이 아니라 추출이라 다르다).

★그리고 **프롬프트 지시를 믿지 않는다.** `_verify_narration_intact()`가 파싱 결과를
원본과 코드로 대조한다. 지시를 어긴 전례가 반복됐기 때문이다(07-23 상품 지퍼가
생겨남, 08-02 케이크 배정 무시). 어긋나면 막지 않고 **경고만** 남긴다 — 여기서
실패시키면 대본을 아예 못 쓴다. 공백·문장부호는 정규화 후 비교한다.

`assign_speaker_voices()` — 화자 이름을 등장 순서대로 성별 후보에서 배정.
성별이 안 잡히면 남녀 번갈아(둘 다 여성으로 몰리면 구분이 안 된다).
★**한국어 외에는 `VOICE_BY_LANG`에 언어당 목소리가 1종뿐**이라 화자를 못 나눈다 —
배정하지 않고 경고만 남기고 전체 공용 목소리로 폴백한다.

### 분기 지점 3곳

새 상태값을 만들지 않았다 — 시나리오 **1개짜리 목록**으로 저장해 기존
select/preview/produce 흐름을 그대로 태운다(회귀 위험 최소).

| 위치 | 동작 |
|---|---|
| `process_research` | `form.userScript`가 있으면 리서치·시나리오 생성을 통째로 건너뛴다(구글 검색 그라운딩 원가도 그만큼 준다) |
| `process_produce` | `_script_from_scenario()`가 대본을 돌려주면 `build_script_draft`를 **안 태운다** — 태우면 내레이션이 다시 쓰인다 |
| `process_preview` | 1장면을 **그대로** 미리보기로 쓴다. LLM 호출 1회 감소 + 미리보기와 완성본의 첫 장면이 같아진다(결제 판단 근거) |

★`_script_from_scenario()`를 함수로 뺀 이유 — produce와 preview가 **같은 판정**을
쓰게 하려는 것. 한쪽만 고치면 미리보기와 완성본의 대본이 달라진다(과거
`plan`/`restyleKeys`가 한쪽에만 전달돼 조용히 어긋난 전례).

★미리보기의 사진 없는 분기는 원래 `biz`/`topic`으로 이미지 프롬프트를 만들었는데,
대본만 받은 경로에서는 이 둘이 비어 있어 **내레이션 문장이 영어 프롬프트 자리에**
들어갈 뻔했다. 1장면의 `scene_prompt`를 쓰도록 고쳤다.

### 결정 사항

- **자막이 길어도 자르지 않는다.** 사장 대본 자막은 22자를 넘는 게 5개인데 그대로 둔다 —
  "대본을 그대로 쓴다"가 이 기능의 취지다. `CAPTION_WARN_LEN` 초과 시 경고만 남긴다.
- **목소리는 자동 배정.** 화면에서 고르는 UI는 ③단계(미착수).
- `MAX_USER_SCRIPT_SEGMENTS=10` — 세그먼트 1개 = 이미지 1장 + TTS 1회라 그대로 원가다.
  ★**요금제별 정식 상한은 미정(사장 결정 사항)** — 지금은 안전 상한만 두고 초과분은
  잘라내며 경고에 남긴다.

### 검증

사장 대본 실측: 장면 6개, **내레이션 6개 전부 원문 그대로**, 지은→Leda(F)/민호→Charon(M) 분리.
회귀 11종 PASS(기존 대본 경로 무변경), 폴백 7종 PASS, 실제 TTS 2종 음성 상이 확인.

### ★남은 일

- **접수 경로가 없다** — 지금은 `form.userScript`를 워커가 읽을 뿐, shared-api와 화면에
  대본을 넣을 자리가 아직 없다(③단계). 현재는 DB에 직접 넣어야 탄다.
- **인물 일관성** — 장면마다 이미지를 따로 생성해 **같은 부부가 다른 얼굴로 나온다**.
  홍보·정보물은 괜찮지만 스토리텔링은 이게 곧 품질이다. 1장면 인물을 참조 이미지로
  물려주는 방식이 유력(GPU 불필요). ④단계.
- 요금제별 장면 수 상한·과금 정책.

## 🎨 이미지 품질 3종 — 국적·인물 일관성·자막 여백 (2026-08-22 2차, 사장 지적)

사장이 같은 대본으로 **ChatGPT가 만든 스토리보드**와 우리 결과물을 나란히 놓고 지적했다.
*"60점에서 70점 정도, 이미지는 한국 동양적이지 않네."* 비교 대상이 있으니 차이가
명확했고, 고칠 것 셋이 그대로 드러났다.

### ① 국적 — 지시 누락이었다

이미지 프롬프트에 **국적이 한 글자도 없었다.** SDXL 계열은 학습 데이터가 서구권에
쏠려 있어 **안 적으면 백인이 기본값**이다. 모델 탓이 아니라 우리가 안 시킨 것이다.

`SCENE_ETHNICITY` / `_ethnicity_clause(lang, english)` — **언어 설정을 그대로 따라간다.**
회원이 이미 고른 값이라 추가 입력이 없고, 영어 쇼츠에 한국인이 나오는 어색함도 없다
(`en`은 강제하지 않는다 = 기존 동작).
★국적명만으론 부족하다 — SDXL이 옷·소품 같은 표면 기호로만 반영하고 얼굴은 서구권
그대로인 경우가 있어 **얼굴 특징까지** 적는다(`Korean people with East Asian facial features`).
gemini(한국어)·GPU(영어) 두 경로가 있어 언어별로 문구를 한 쌍씩 둔다.

### ② 인물 일관성 — ★체인은 원래 있었는데 안 돌고 있었다

이게 이번 작업의 핵심 발견이다. `last_image_bytes`로 **직전 장면 결과를 다음 장면 참조로
물려주는 체인이 이미 구현돼 있었다.** 그런데 사진 없는 카테고리에서는 한 번도 작동한
적이 없다.

```
raw_list 비어 있음 → last_image_bytes = None
                   → _regenerate_scene(src_bytes=None)
                   → GPU(text2img) 분기 → 참조를 아예 안 씀
                   → 다음 세그먼트도 같은 상태 → 체인이 시작조차 못 함
```

전 장면이 독립 생성돼 **매번 다른 사람**이 나왔다. 첫 장면만 그런 게 아니라 끝까지.

→ `_chainIdentity` 플래그. 켜지면 **1장면만 GPU**(원가 이점 유지), 2장면부터 gemini
참조 경로로 보낸다. 파싱 프롬프트에도 *"인상착의(나이대·머리모양·옷차림)를 등장할
때마다 똑같이 반복해 적어라, 배경도 같은 장소면 같은 말로"*를 넣어 **텍스트 수준에서도**
묶는다(실측: `A husband` → `A man in his 30s with short dark hair, wearing casual clothes`가
1·5·6장면에 동일하게 반복됨).

### ③ 자막 여백 — 스토리보드에서 배운 것

우리는 이미지를 다 그린 뒤 **그 위에 자막을 얹는다.** 그래서 얼굴이나 중요한 소품을
자막이 덮는 일이 생긴다. 잘 만든 스토리보드는 **처음부터 자막 자리를 비워두고** 그린다.

`_reserveCaptionSpace` → 구도의 1/3을 단순 배경으로 비워 달라고 요청하고, 장면마다
**좌/우/하로 번갈아** 배치한다(한쪽만 계속 비우면 화면이 단조로워진다).
★"비우라"만 강하게 주면 인물이 구석에 몰리므로 *"인물은 반대쪽에 크게, 잘리지 않게"*를
같이 적는다.

### ★②③은 스토리형에서만 켠다 — 원가 때문

참조 경로는 GPU가 아니라 API를 탄다. **장당 5원 → 90원**(6장면이면 30원 → 480원).
홍보·정보 카테고리는 장면마다 다른 사람이 나와도 무방하고 오히려 다양성이 이득이라
기존대로 둔다. `parse_user_script`가 두 플래그를 켜므로 **가져온 대본만** 적용된다.
★정식 요금제 반영은 사장 결정 사항 — 코드에만 두고 과금은 안 건드렸다.

### 검증 방법

★**"코드에 넣었다"와 "프롬프트에 들어간다"는 다르다.** 가짜 클라이언트/가짜 gpu_image로
**모델에 실제 전달되는 문자열을 포착**해 확인했다(프롬프트 도달 11종 PASS).
체인은 `_regenerate_scene`을 가로채 호출 인자를 기록해 *1장면 참조없음 → 2~5장면 참조있음*
순서를 실측(7종 PASS, 기존 카테고리가 전 장면 참조없음으로 남는 것도 함께 확인).

## 🩹 자막 겹침 수정 + 여백 지시 철회 (2026-08-22 3차, 사장 실화면 지적)

사장이 2차 영상을 보고 *"자막하고 문구가 겹쳐지는 거 같은데"*라고 지적했다. 맞았다.

### ★내 오해 — 자막은 이미지 위에 얹히지 않는다

2차에서 넣은 "구도 1/3을 자막 자리로 비워라"는 **전제부터 틀렸다.** 실제 레이아웃은:

```
[상단 96]     브랜드 배지
[280~1220]    이미지 카드          ← 자막은 여기 안 들어온다
[1308~]       caption(76pt 흰 글씨)
[하단]        내레이션 칩(56pt, 검은 반투명)
[H-140]       진행 점
```

자막은 **이미지 카드 아래 별도 영역**이다. 이미지 안에 여백을 만들어도 겹침과
무관하고, 그림에 빈 공간만 생겨 구도가 헐거워졌다(2차 3번 장면 오른쪽 회색 여백).
→ `_reserveCaptionSpace = False`로 철회.

★**이미지만 보고 "자막 자리가 확보됐다"고 판정한 것이 잘못이었다.** 완성 프레임을
뽑아 봤어야 했다. [[feedback_verify_actual_effect]]

### 진짜 원인 — caption 1줄을 가정한 레이아웃

좌표로 계산하면 명확하다.

| | 시작 | 끝 |
|---|---|---|
| caption 1줄 | 1308 | 1412 |
| caption 2줄 | 1308 | **1516** |
| 내레이션 3줄 칩 | **1468** | 1716 |

**caption 2줄 + 내레이션 3줄 = 50px 겹침.** 사장 대본 자막이 25자라 2줄이 됐다.
★가져온 대본만의 문제가 아니다 — **기존 쇼츠도 긴 자막이면 똑같이 겹쳐 왔다.**

### 고친 방법 (`make_short.py` render_frame)

두 단계로 나눴다. 칩을 밀어내기만 하면 내레이션이 3줄→1줄로 잘려나가기 때문이다.

1. **카드 높이 양보** — caption이 한 줄 늘 때마다 이미지 카드를 104씩 줄인다
   (940 → 836 → 732, 최소 700). 자리를 만들어 주는 게 근본이다.
2. **칩 밀어내기(안전망)** — 그래도 겹치면 `cap_bottom + 16`으로 내리고,
   진행 점까지 넘치면 그때만 줄 수를 줄인다.

★**caption 1줄이면 카드 높이 940 그대로** — 기존 쇼츠 화면은 1픽셀도 안 바뀐다.

### 검증 — 실제 프레임으로

`render_frame`을 직접 호출해 완성 PNG를 만들고 **눈으로 확인**했다.

| 케이스 | 카드높이 | 간격 | 내레이션 |
|---|---|---|---|
| 자막 1줄(기존 쇼츠) | 940 | 56px | 3줄 → 3줄 |
| 자막 2줄(사장 대본) | 836 | 56px | 3줄 → 3줄 |
| 자막 3줄(극단) | 732 | 56px | 3줄 → 3줄 |

세 경우 모두 겹치지 않고 **내레이션이 온전히 유지**된다.

★첫 시도(칩 밀어내기만)는 겹침은 없앴지만 내레이션이 3줄→1줄로 잘렸다. 렌더한
프레임을 보고서야 알았다 — 수치 검증만 했으면 "PASS"로 넘어갔을 것이다.

---

## 🩹 자막 겹침 **재수정** (2026-08-26, 사장 재지적 *"저번에도 그러더니"*)

위 8/22 조치로 끝난 줄 알았으나 **8/22 산출물부터 이미 겹쳐 있었다.** 사장이 다시 지적했다.

### 왜 8/22 검증을 통과했는가 — 계산과 실제가 달랐다

위 표의 "간격 56px"은 **계산값**이다. 완성 프레임을 픽셀로 재면 다르다.

| | 8/22 계산 | 2026-08-26 실측 |
|---|---|---|
| caption 2줄 끝 | 1412 | **1464** |
| 칩 상단 | 1468 | 1468 |
| 간격 | 56px (안전) | **4px** (받침·물음표 꼬리가 먹힘) |

**원인**: `cap_bottom = cy` 인데 `cy` 는 마지막 줄을 그린 뒤 `+104` 한 값이라
**"다음 줄이 시작될 자리"**이지 **글자의 실제 아랫변이 아니다.** 76pt 한글은 줄간격보다
아래로 더 내려간다. 계산이 **56px 낙관적**이었고, `MIN_GAP=16` 이 그 오차를 못 덮어
코드가 **"안 겹친다"고 오판**하고 칩을 밀지 않았다.

### 고친 것 (`make_short.py`)

1. **`cap_bottom` 을 실제 렌더 bbox 하단으로** — 줄마다 `draw.textbbox(...)[3] + 3` 을
   `max` 로 누적한다. 실측 오차 ±1px 로 정확하다.
2. **`MIN_GAP` 16 → 24** — 4px 까지 붙어도 통과시키던 것을 차단.
3. **칩이 없으면 카드를 줄이지 않는다**(`_has_chip` 판정) — 카드 축소는 **칩에 자리를
   내주려는 계산**인데 칩이 없는 세그먼트에서도 줄여 하단이 휑하게 비었다.

### 팟캐스트형은 칩 자체를 뺀다 (사장 지시)

caption(흰 큰 글씨)과 칩(내레이션 전문)이 **앞부분이 그대로 중복**됐다.
사장 판단으로 **팟캐스트형에서만 칩을 제거**했다.

★**운영 코드는 수정하지 않았다** — 조립기에서 `seg["text"]` 를 **비워 넘기면**
`make_short` 가 알아서 생략한다(`if seg.get("text") and not seg.get("qr_url")`).
다른 카테고리·기존 쇼츠는 종전대로 칩이 나온다.

### 검증 — 이번엔 픽셀로

| 케이스 | 결과 |
|---|---|
| 칩 있음(2줄 자막) | 자막 1216~1386 / 칩 1468 → **82px 여유** ✅ |
| 팟캐스트형 6장면 | 칩 없음·자막 온전(순검정 0, 배경 RGB 30 균일) ✅ |
| 회귀 1줄+칩 | card_h **940** (종전 동일) ✅ |
| 회귀 2줄+칩 | card_h **836** (종전 동일) ✅ |

★★**레이아웃 겹침은 계산이 아니라 픽셀로 확인한다.** 계산상 "80px 여유"가 나오는데
화면은 겹쳐 있었다. `np.array(Image.open(...))` 로 흰 글씨/검정 칩의 y 범위를 직접 잰다.
⚠️단 **칩 안의 흰 글씨를 자막으로 세지 말 것** — 칩 상단 위쪽만 탐색해야 한다
(이걸 놓쳐 "자막이 1691까지"로 읽고 한 번 더 오진했다).

⚠️`shorts-factory` 는 **git 미연동**이다. 백업은 `make_short.py.bak-20260826` 파일뿐.

## 📥 대본 가져오기 접수 경로 신설 (2026-08-22 4차 — 기능 완성)

워커(rag)만 있고 **넣을 자리가 없던 것**을 화면·API까지 이었다. 이제 회원이 직접 쓴다.

### 화면 (`ShortsMakerBoard.tsx`)

사진 없는 카테고리(insight/wellness/meme)에 전환 탭을 넣었다.

```
[ ✏️ 주제만 입력 ]  [ 📄 대본 가져오기 ]
```

- textarea 붙여넣기 + **.txt/.md 파일 불러오기**(200KB·6,000자 상한)
- 글자수 카운터, *"내레이션은 한 글자도 바꾸지 않고 그대로 씁니다"* 안내
- 화자 여러 명이면 이름을 적으라는 힌트(남녀 목소리 자동 분리)

★**topic과 userScript는 배타 전송** — 둘 다 보내면 서버가 대본 분기를 타면서
화면에 남아 있던 옛 주제가 formJson에 섞인다. 안 쓰는 쪽을 `delete`한다.
★**카테고리를 사진 필요한 쪽으로 옮기면 자동으로 꺼진다** — 켜진 채 남으면 사진
카테고리에서 대본이 조용히 함께 전송된다. `reset()`에서도 해제.

### API (`shared-api/routes/aimp/shorts-maker.ts`)

- `FIELD_CAPS.userScript`(6,000자) — 6장면 기획안이 1,500~2,500자라 넉넉히
- 대본이 있으면 **topic/biz/strengths/target 필수 검사를 면제** — 그 값들은 시나리오를
  *생성*하기 위한 재료인데 이 경로는 생성 단계 자체를 건너뛴다
- 50자 미만 거부 + 사진 없는 카테고리 전용 제한(화면과 같은 조건을 서버에서도)

### ★검증 — 운영을 안 건드리고 별도 포트로 먼저

`PORT=3099`로 새 코드를 띄워 전부 실측한 뒤에 운영을 reload했다.
[[feedback_dev_env_db_network]]의 무중단 검증법 그대로.

| 검증 | 결과 |
|---|---|
| 짧은 대본 / 사진 카테고리 / 주제·대본 둘 다 없음 / 6,000자 초과 | 전부 400 |
| **사장 대본 접수** | **202, id 41** |
| 크론 워커 자동 처리 | `scenarios_ready`, 시나리오 1개("직접 작성한 대본") |
| 파싱 결과 | 장면 6개, 지은→Leda(F)/민호→Charon(M), 체인 ON·여백 OFF |

화면 → API → DB → 워커 → 파싱까지 **전 구간 실동작 확인**. 테스트 접수(#41)와
임시 서버는 정리했다.

### ★자동배포가 이 커밋을 못 잡는다 — 스크립트 결함 아님

서버1에서 **직접 커밋하고 거기서 push**하면 `로컬 == 원격`이 되어
`shared-api-autodeploy.sh`가 "변경 없음"으로 조용히 종료한다(29~35줄).
→ 이 경우는 **`pm2 reload shared-api`를 수동으로** 해야 한다.
서버2에서 push하는 평소 흐름에서는 정상 작동한다. [[project_server1_uncommitted_code]]

### 남은 일

- **인물별 참조 분리** — 체인이 직전 장면을 무조건 참조해서, 부부 장면 다음에
  전문가를 그릴 때도 아내 얼굴을 참조한다. 지금은 결과가 괜찮지만 운에 기댄다.
- **FaceID 연결(원가)** — 참조 경로가 API라 장당 5원→90원. 서버3 FaceID를 붙이면
  GPU로도 인물 유지가 되어 되돌릴 수 있다. 사장 지시로 보류. [[project_faceid_server3]]
- **요금제별 장면 수 상한·과금** — `MAX_USER_SCRIPT_SEGMENTS=10`은 안전 상한일 뿐.
