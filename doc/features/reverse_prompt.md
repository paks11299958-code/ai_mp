...
| `ReversePromptLibrary.tsx` | S2 — 보관함(목록·상세·삭제·**썸네일 확대 모달**). **로그인 전용** |
...
### 썸네일 확대 모달 (2026-08-18 추가)

목록의 썸네일을 누르면 확대 모달이 뜬다. 닫기는 **ESC · 배경 클릭 · × 버튼** 3경로이고,
열려 있는 동안 배경 스크롤을 잠갔다가 닫히면 원래 값으로 되돌린다.

- ★**썸네일은 펼치기 버튼 밖에 둔다** — 버튼 안에 버튼을 중첩하면 HTML이 무효라
  브라우저가 DOM을 재구성한다. 썸네일=확대, 나머지 영역=아코디언으로 역할을 나눴다.
- ★**모달 내부 래퍼는 `inline-block`이어야 한다.** `max-w-[92vw] max-h-[86vh]`만 주면
  그 블록이 화면을 거의 덮어, **배경처럼 보이는 여백을 눌러도 `stopPropagation`에 막혀
  안 닫힌다**(2026-08-18 실측으로 발견 — 눈으로는 배경과 구분되지 않는다).
- ★**확대해도 원본 화질이 아니다.** 보관하는 이미지는 **128px 썸네일뿐**이고 원본은
  정책상 저장하지 않는다. "어떤 이미지였는지 확인"하는 용도이며, 모달 하단에 그 사실을
  안내한다. 원본 화질을 원하면 원본 저장 정책부터 바꿔야 한다(개인정보 원칙과 충돌).
