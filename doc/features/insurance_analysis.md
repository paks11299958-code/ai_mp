...
## 흐름 (비동기 큐 — 명품검증 패턴)
1. 김지훈 채팅 기능아이콘 **🛡 보험 분석** → `InsuranceBoard` 오픈
2. 기본정보 입력(제목·성별·생년월일 + 직업·건강·예산·목적 선택). 생년월일은 **연/월/일 드롭다운 + 양력/음력 체크**
3. 보험증권 **PDF/이미지** 업로드(GCS signed-url, 최대 5개)
4. "AI 중복 분석 시작" → 포인트 차감 → `InsuranceAnalysis` 레코드 `pending` 생성 → 화면 맨 위로 스크롤(진행상황)
5. **서버1 cron(insurance-worker 매분)** 이 처리 → 완료 시 결과 카드(중복항목·절감액·위험도 + 상세 + AI권고)
6. 결과 화면에서 **종합 컨설팅 받기**(영구 보고서) / **채팅 상담**(추가 질문) / **인쇄·PDF**

## 2단계 분석 (Gemini 2.5-flash)
worker(`internal-cron.ts` insurance-worker):
- **1단계 추출**(insExtract): PDF/이미지를 `fileData:{fileUri, mimeType}`로 Gemini Vision에 직접 전달 → 증권별 보장항목 추출(원문 그대로, 창작 금지). PDF는 `application/pdf` mime(insGuessMime, 상단 guessMime은 이미지전용이라 별도)
- **2단계 중복분석**(insAnalyze): 추출 데이터만으로 중복(완전/부분/유사) + 절감액 + severity + 권고 도출. 가입자 정보(만 나이 환산·음력 표기) 반영
...
- 프롬프트(CONSULTING_INSTRUCTION)가 마크다운 4섹션 강제: ##1 중복·절감 요약 / ##2 보완 보장 / ##3 절감액 활용 / ##4 추가 확인. 불릿·번호·굵게
- 프론트: react-markdown 렌더(`.ins-report` 스타일, index.css) + 접기/펼치기. 인쇄·PDF에도 포함
- 채팅 휘발이 아니라 **분석 1건 = 영구 컨설팅 문서**(재방문 시 항상 표시)
...
## 파일
- 백엔드(shared-api): `routes/aimp/insurance-analysis.ts`(upload-urls/POST/GET/:id/retry/consulting/DELETE) + index.ts 등록 + `internal-cron.ts` insurance-worker + prisma `InsuranceAnalysis` 모델
