...
## 📝 페이퍼봇 전용 탭 — mode 분기 재사용 (2026-08-05)

사장 지시 "실봇 메뉴들 있잖아, 그거랑 똑같이 페이퍼봇 메뉴도 만들어".
어드민 시스템 그룹 **'가상매매(페이퍼)'** 탭 (`mainView='paper-trader'`).

★**복제하지 않고 `TossTraderPanel`에 `mode?: 'live'|'paper'` 인자를 추가해 재사용**했다.
`status_paper.json`이 실봇 status와 **46개 키까지 완전히 동일**(실측)해 화면 로직이
그대로 통한다. 1795줄을 복제하면 모든 수정이 2배가 되고 그 2배는 곧 어긋난다
(페이퍼/실봇 **소스 분리를 하지 않기로 한 결론**과 같은 이유 — 아래 참조).

**구성: 읽기전용 6탭** (발굴·수익률·모니터링·평가·로그·설정)
- ★'선택' 탭 제외 — `auto_select`가 매일 실봇 발굴 추천으로 덮어써 수동 선택이
  다음 스캔에 사라진다. **눌리는데 사라지는 버튼은 없느니만 못하다.**
- 쓰기 7곳 차단(선택저장·halt 2·재시작 2·커스텀종목 2). ★UI에서 버튼을 감추는 것에
  더해 **함수 진입부에서도** 막았다(우회 경로 차단). `custom_symbols.json`은 실봇과
  **공유 자산**이라 페이퍼 화면에서 고치면 실봇 유니버스까지 바뀐다.
- 정지/재시작 버튼 미노출 — 가상매매라 실익이 없고, 실봇 재시작(위험작업 4종)과
  버튼 모양이 같아 오조작 위험만 생긴다.

**데이터 소스 분기**: status/logs/orders/selection은 `*_paper`, 발굴(scan_results)·
채원 일기·커스텀 종목은 실봇이 만들어 페이퍼가 읽기만 하는 **공유 자산**이라 동일 API
(`paths.py`의 `base_path` 원칙과 같다).

**백엔드**: `GET /paper/selection` 신설(읽기 전용 — 쓰기 API는 의도적으로 안 만듦),
`/paper/orders`·`/paper/performance`(성과 요약: 평가자산·수익률·일별손익·체결수 +
감시종목 점수 대 임계).

**부수 버그 수정(실봇에도 적용)**: ★`PAPER`를 '드라이런(DEBUG)'으로 표기하던 곳 2군데
정정. 둘은 다르다 — DEBUG=관찰용 드라이런, PAPER=**가상계좌에 손익이 누적되는 실험**.

**검증**: ★tsc·빌드는 런타임을 못 잡으므로(TDZ 백지 사고) **실제 렌더링까지** 확인.
프로덕션 DB 접속이 막힌 환경이라 임시 라우트로 컴포넌트를 격리 마운트 + 실제 서버1
데이터를 목으로 먹여 Playwright 실측 — 탭 6개(선택 없음), 5개 탭 전환 모두 정상 렌더
(수익률 2,905,523원/0.00%, 모니터링 한도 290,552원, 평가 60점/임계70, 설정 코웨이),
실패요청 0·페이지에러 0. 검증 코드는 완전 원복(git diff 없음 확인).

### 페이퍼/실봇 소스 분리 검토 → 하지 않기로 (2026-08-05)

사장 질문 "페이퍼봇과 리얼봇 소스를 분리하는 게 어때?" → 실측으로 판단:
- `cfg.paper` **실행 분기는 단 3곳**(`broker.py:105`·`133` 시뮬 포지션 파일 영속화,
  `executor.py:244` 격리 장부 기록) + 로그 문구 2곳뿐.
- ★**매매 판단 로직(`strategy.py`·`risk.py`·`scanner.py`)엔 paper 분기가 0개** —
  두 봇이 판단을 100% 동일하게 하는 것이 페이퍼 봇의 **존재 이유**이므로 현 구조가
  목적에 부합한다. 분리하면 3줄 때문에 20+ 파일을 복제하고 전략 수정마다 어긋난다.
- **재검토 신호**: 페이퍼에서만 *다른 전략*을 A/B 하고 싶을 때.
