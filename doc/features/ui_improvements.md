...
## 회원 화면 수리 3건 — 사장이 실사용 중 발견 (2026-08-08)

셋 다 **개발·데스크톱에서는 절대 안 보이는** 종류였다. 실제 폰으로 가입해 보고서야 드러났다.

### ① 인증번호 입력창이 모바일 키보드에 가려졌다 (`AuthModal.tsx`)

증상: "휴대폰 인증번호가 왔는데 **입력창이 가려져서 안 보임**".

원인이 **둘**이었다.
- **잘림** — `flex-1 flex flex-col justify-center`는 **스크롤 없는 세로 중앙정렬**이라,
  키보드가 올라와 뷰포트가 **844→400px**로 줄면 내용이 위아래로 잘리고 **꺼낼 방법이 없다**
  (실측: 카드 629px 중 **245px 잘림**, 래퍼 `overflow: visible`). 인증번호 칸은 화면 아래쪽이라
  **정확히 그 구간**에 들어간다.
  → `overflow-y-auto overscroll-contain` + **`my-auto` 래퍼**.
  ★`justify-center`를 남겨두면 **넘칠 때 위쪽이 잘려 스크롤해도 못 올라간다**
  ("짧으면 가운데, 넘치면 위부터 스크롤"이 되게 my-auto를 쓴다).
- **포커스** — `autoFocus`는 이미 있었는데도 안 갔다. 이 입력창은 **조건부 렌더로 나중에 마운트**되고,
  모바일은 제스처와 분리된 autoFocus를 무시하기도 하며, **포커스가 가도 줄어든 뷰포트 밖이면 여전히 안 보인다**.
  → `ref`로 `focus({preventScroll:true})` + `scrollIntoView({block:'center'})`, **중첩 rAF**로 레이아웃 후 실행.
- 덤: 같은 input에 `autoComplete`가 **중복 선언**돼 뒤의 `"off"`가 이기고 있었다 → `one-time-code`
  (문자 인증번호가 키보드 위 자동완성으로 뜬다).

★**엉뚱한 곳을 먼저 고쳤다**: 이 파일엔 인증번호 입력창이 **두 군데**라 모달 래퍼를 먼저 손댔는데,
실제 회원가입은 **`fullScreen` 분기**를 탄다. 단서는 재현 스크린샷의 **좌상단 뒤로가기 화살표**였다
(모달이면 우상단 X). **코드를 고치기 전에 화면을 먼저 찍어 분기를 확정한다.**

### ② 생년월일·궁합 모달의 다크 테마 잔재 (`BirthInfoModal.tsx` / `PartnerInfoModal.tsx`)

증상: "명부등록할 때 생년월일 모달창이 **너무 어두워서 글씨도 안 보임**".

모달 본체는 이미 흰색 계열(`linear-gradient(170deg,#FFFFFF,#F5F0FB)`)인데 **일부 상수만 어두운 값**이라
명도 차가 사라졌다.
- `selectStyle`: `background: rgba(20,14,6,0.95)`(거의 검정) + `color: #3F3350`(어두운 보라)
  → **검정 위 어두운 보라**라 연/월/일이 배경에 묻혔다. → 흰 배경으로.
- ★**결정적 단서는 같은 파일 안의 불일치**였다: `.birth-select option`은 **이미 밝은 값**이라
  **펼친 목록과 닫힌 상태의 색이 서로 달랐다.** 한 화면에서 톤이 엇갈리면 "디자인 의도"가 아니라 **누락**을 의심한다.
- 궁합 모달은 더 심했다 — 휠 위아래 **'안개'가 검정**(`rgba(13,11,8,0.98)`)이라 **선택 항목 앞뒤 연도가
  통째로 묻혀** 있었다. 안개는 **배경색과 같아야** 한다 → `rgba(255,255,255,0.98)`.
- '나중에'/'취소' 버튼도 같은 잔재(흐린 회색 배경 + 밝은 회색 글씨) → 대비 확보.

★**일괄 치환하지 않았다**: 같은 `rgba(20,14,6,…)`가 관상·손금 모달에도 있지만 **거기는 의도된 다크 테마**다.
대비 문제는 "이 값이 나쁘다"가 아니라 **"이 배경 위에서 나쁘다"**이므로 반드시 **그 컴포넌트의 배경과 짝지어** 판단한다.

### ③ 로그인·회원가입·명부 모달에 ErrorBoundary가 없었다 (`App.tsx`)

사장 질문: "에러 났을 때 안내 모달도 처리했니?"

★`ErrorBoundary`는 전자책·웹툰·헤어 등 **23곳에 이미 적용**돼 있어 "되어 있다"고 넘어가기 쉬웠는데,
정작 **로그인/회원가입(`AuthModal` 3곳)과 생년월일 명부(`BirthInfoModal`)**에는 없었다.
여기서 렌더 에러가 나면 **안내 없이 앱 전체가 죽는다**(2026-07-28 백지 사고와 같은 형태).
특히 회원가입은 **신규 회원이 처음 만나는 화면**이라 죽으면 가입 자체가 불가능해진다.

적용(각각 `onClose`로 원래 화면 복귀 — 사용자가 갇히지 않게):
`authPage`(전체화면 로그인) / 추천링크 가입 실패 경로 / `showAuthModal`(모달) / `showBirthModal`(명부).

★**"감쌌다"≠"잡힌다"라 실제로 터뜨려 검증했다** — `AuthModal` 렌더 최상단에 강제 예외를 **일시 주입해
빌드**하고 진입 → 앱이 죽지 않고 "⚠️ 로그인 화면 오류 / 다시 시도 / 닫기"가 뜨는 것과 콘솔의
`[ErrorBoundary] 렌더 에러` 로그를 확인. 이후 **원본 복원 + 소스·번들 양쪽에서 테스트 코드 0건** 확인 후 커밋.
※전역 API(`Array.map`/`Object.entries`) 후킹으로 재현하려다 두 번 실패했다 —
`ErrorBoundary`는 **렌더 단계** 에러만 잡고 **이벤트 핸들러 예외는 원래 안 잡는다**(정상 동작).

**검증 공통**: 로컬 프리뷰의 **빌드 번들**에 대고 390×400(키보드 상태) 뷰포트로 실제 진입해 측정
(`/auth/send-verify`만 라우트 가로채 SMS 미발송). 운영 배포 후 Playwright로 재확인.
★프리뷰가 죽어 **옛 번들**을 보고 "수정이 안 먹었다"고 두 번 오판했다 — **서빙되는 asset 해시를
방금 빌드한 파일명과 대조**하고 재야 한다.

---

## AI 스튜디오 화면 개편 — 2단 배치·사용량 표·썸네일 축소 (2026-08-08)

사장이 첨부한 화면대로 어드민 [시스템 > AI 스튜디오]를 재배치했다.
**기능 추가가 아니라 "쓰기 편하게"가 목적**이라, 무엇이 왜 헷갈렸는지를 함께 남긴다.

### ★한 줄로 늘어선 폼은 "쓰려면 지나쳐야 하는" 구조가 된다
생성 폼이 세로 한 줄이라 **프롬프트를 쓰려면 프리셋·사진업로드를 한참 지나쳐야** 했다.
→ 2단으로 나눴다: **왼쪽=재료**(프리셋·내 사진·스타일 견본) / **오른쪽=지시**(그리고
싶은 것 → 빼고 싶은 것 → 모델·크기·스텝·장수 → 생성). `lg` 미만은 자동으로 한 줄.

`lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)]` — ★`minmax(0,…)`가 없으면 내부의 긴
텍스트가 열을 밀어내 가로 스크롤이 생긴다.

★**옮기기만 할 때는 기계적으로 대조한다**: 핸들러·상태 식별자(`on*`/`do*`/`set*`)의
**개수**를 이전 파일과 비교해 전부 일치를 확인했다. 눈으로 훑으면 한 블록이 빠져도 모른다.

### ★버튼은 "붙어 있는 것"에 속한다 — 가운데 두면 소속이 모호해진다
`✨ AI로 다듬기`가 두 텍스트창 **사이**에 있어 사장이 "이거 누르면 네거티브도 영어로
바뀌나?"라고 물었다. **실제로는 위 칸만** 바꾼다(`doRefine`이 `prompt`만 읽고 쓴다).
→ 제목·입력칸·버튼을 **한 덩어리(`space-y-1.5`)로 묶고**, `space-y-3`으로 벌어져 있던
간격을 없앴다. 문구도 "**위 칸을** AI로 다듬기"로 바꿔 대상을 못박았다.

### 네거티브 다듬기 신설 — ★의미가 정반대라 같은 지시문을 쓰면 안 된다
사장 지적("네거티브도 한글로 쓰는 사람이 있을 수 있다")으로 같은 버튼을 아래 칸에도
붙였다. 단 서버 지시문은 **분리**해야 한다(`mode='negative'`):
"손가락이 이상하지 않게"를 주어 묘사로 번역하면 **오히려 그걸 그린다.**
→ negative 모드는 `deformed hands, extra fingers` 같은 **회피 태그**로 뽑는다.

★`doRefine`을 `mode` 인자로 **일반화**했다 — 복사해 함수를 둘로 만들면 나중에
한쪽만 고치는 실수가 난다.

### 일자별 사용량 표 — ★"오늘"만 보면 추세를 알 수 없다
최근 14일의 날짜·건수·사용시간·**금액(천 단위 콤마)** + 합계.
★**금액은 `elapsedSec`(그림 그린 시간) 기준이라 실제 청구액보다 작다** — 서버가 켜져
있어도 노는 시간은 안 잡힌다. **화면에도 그렇게 적었다**(정확한 건 GCP 결제 화면).
숫자 열은 `tabular-nums` — 세로로 비교해야 하는 값이라 자릿수가 맞아야 한다.

### 썸네일 1/4 + 클릭 확대 + 칸별 삭제
4열 → **8열**(`grid-cols-4 sm:grid-cols-6 lg:grid-cols-8`). 한 장이 너무 커서 훑기
어려웠다. 작아진 대신:
- **클릭하면 확대** — 배경 아무 곳이나 눌러도 닫힌다(닫는 법을 못 찾으면 갇힌 느낌).
  확대 화면에서 내려받는다(예전엔 **클릭이 곧 다운로드**라 크게 볼 방법이 없었다).
- **✕ 삭제** — 평소 `opacity-40`, `group-hover`로 또렷하게. 칸이 작아 항상 진하면 그림을 가린다.
  ★**DB 기록만** 지운다. 서버3 파일은 남아 [AI 보관함]에서 관리(원본이 사라지면 사고).
  ★낙관적 갱신 후 **실패하면 되돌린다** — 지워진 것처럼 보이면 안 된다.

### ★JSX 함정 — 삼항 `: (` 뒤에는 주석을 못 넣는다
```jsx
{cond ? (<A/>) : (
    {/* 주석 */}      ← ★빌드 실패: 그 자리는 표현식만 가능
    <B/>
)}
```
`Expected ',' or ')' but found Identifier`로 죽는다. 주석은 **삼항 밖으로** 뺀다.
`npm run check`는 통과했고 **`npm run build`에서만** 잡혔다.

### 검증
`npm run check`·빌드 통과 + ★**Playwright 격리 마운트로 실제 렌더 확인**
(프로덕션 DB 접속이 막혀 로그인 재현 대신 컴포넌트만 띄움 — [[feedback_dev_env_db_network]]).
페이지 에러 0 / 사용량 표·천단위 표기 / 다듬기 버튼 2개 / 썸네일 8열 / 삭제버튼 8개 /
1600·1280·900px 전부 가로 스크롤 없음.

**커밋**: ai_mp `5a046bb`(2단 배치)·`44e7417`(사용량·썸네일·다듬기) /
shared-api `7b9d442`(usage·job 삭제·negative 모드 API)

## 어드민 탭·패널 스크롤 3건 + AI 스튜디오 프리셋 (2026-08-05)

### ★어드민 패널은 **자기 스크롤을 스스로 가져야 한다**
`AdminPanel` 본문이 `overflow-hidden` 이라, 패널 루트에 `flex-1 overflow-y-auto` 가
없으면 **내용이 길어지는 순간 아래쪽이 통째로 안 보인다**(스크롤바조차 없어
"메뉴가 없다"로 보인다). 기존 패널은 전부 이 형태인데 **AI 스튜디오·샘플보관함
2개가 빠져** 있었다. 사장이 "아래 메뉴가 안 보인다"고 해서 발견.

→ **새 패널을 만들 땐 루트 클래스를 기존 패널에서 복사할 것.**

실측(A/B, 빌드 CSS 를 물려 실제 중첩 구조에서):
`수정 전 scrollH 1005/clientH 1005 · scrollTop 0` → `수정 후 1005/644 · scrollTop 361`

★**CSS 효과 검증은 빌드 산출 CSS로** 해야 한다 — 개발서버로 확인하면 마크업에
클래스는 붙는데 **computed style 이 `visible`** 로 나와, 멀쩡한 수정을 "안 먹는다"고
오판한다. 그리고 **수정 전 상태로 되돌려 증상이 재현되는지** 봐야 "고쳐서 된 것"인지
"원래 되던 것"인지 구분된다.

### 하위탭 가로 스크롤
콘텐츠 그룹은 탭이 15개라 `flex-wrap` 이면 **3줄까지 쌓여 본문이 밀렸다.**
한 줄 고정(`flex-nowrap overflow-x-auto` + 버튼 `shrink-0`)으로 바꿔 탭이 늘어도
높이가 안 변한다. 활성 탭은 `scrollIntoView` 로 끌어온다(그룹 전환 시 화면 밖에
있으면 "눌렀는데 아무것도 선택 안 된 것처럼" 보인다).
스크롤바는 숨기지 않고 **얇게(6px) 보이게** 둔다 — 탭이 더 있다는 사실 자체가 신호다.

### 서버 꺼졌을 때 "일부만 보이는" 미리보기
AI 스튜디오 이미지는 서버3에만 있고 서버3은 유휴 30분에 꺼진다. 꺼진 뒤엔 미리보기를
못 가져오는데 **브라우저 캐시(1시간)에 남은 것만 보여서** "어떤 건 보이고 어떤 건
안 보이는" 상태가 된다 — 버그처럼 보이지만 정상 동작이다.
문제는 안내 문구가 `gray-600` 이라 어두운 배경에 **묻혀 이유가 안 보였던 것**.
→ 읽히는 색(🌙 + `gray-300`)으로 바꾸고 상단 배너로 캐시 때문임을 알린다.
★서버가 꺼진 걸 아는데 요청하지 않는다 — 이미지 수만큼 헛된 SSH 시도가 나가고 있었다.

### AI 스튜디오 스타일 프리셋 7종
같은 GPU·모델인데 결과 편차가 컸던 원인이 ①모델선택 ②촬영용어 ③비율 셋이라,
검증된 조합을 코드로 고정하고 사장은 **'무엇을 찍을지'만** 입력하게 했다.
★최종 프롬프트를 펼쳐 볼 수 있게 한다 — 안 보이면 결과가 마음에 안 들 때
무엇을 고쳐야 할지 알 수 없다. 상세: `doc/features/ai_studio_gpu.md` §7-2·7-3.
...
## 버튼 입체감 — "버튼처럼 안 보인다" 재지적 (2026-07-29, 사장)

홈 버튼을 색만 진하게 한 1차 수정 후 다시 "스톤 버튼처럼 해달라"는 지적을 받았다.
★**색 대비를 올린 건 평평한 걸 더 진하게 칠한 것**이지 근본 해결이 아니었다 —
사람이 "누를 수 있다"고 느끼는 건 색이 아니라 **깊이 신호**다:
①아래 그림자(표면에서 떠 있음) ②위 밝고 아래 어두운 그라데이션(빛 받는 입체)
③누를 때 그림자가 줄고 내려앉음(물리적 반응). 1차엔 이 셋이 하나도 없었다.

**적용은 차등으로 했다** — 전체를 같은 강도로 올리면 안 되는 이유:
- 입체의 효과는 **주변이 평평하기 때문에** 나온다. 전부 튀어나오면 아무것도 강조되지 않는다.
- 기능 버튼은 소재마다 **고유 팔레트**(color/bgColor/borderColor)라 색마다 그림자를
  따로 조율해야 자연스럽다. 일괄 적용하면 어떤 색은 탁하고 어떤 색은 뜬다.
- 사이트 톤이 아이보리+파스텔로 담백하다. 강한 입체는 게임 UI 인상이라 결이 달라진다.

| 대상 | 처리 |
|------|------|
| 헤더 홈 버튼 (하나뿐·중요) | **강한 입체**: 보라 그라데이션 + 아래 3px 단차 + 드롭섀도, 누르면 3px 내려앉음 |
| 채팅 기능/퀵메뉴 버튼 (5~6개 나열) | **은은한 입체**: 위 하이라이트 + 아래 2px 단차, 색은 건드리지 않음 |
| 메인 기능 카드 | **눌림 반응만** — 이미 그림자·호버 리프트가 있어 입체감은 충분했다 |

★**잘 잡힌 디자인은 건드리지 않는다.** 메인 카드는 `0 6px 20px` 그림자와 `translateY(-5px)`
호버가 이미 있었고 없던 건 눌림 반응뿐이었다 — 확인 없이 일괄 적용했으면 오히려 망쳤다.

**검증**: 운영 URL에서 `getComputedStyle(el).boxShadow`로 실제 적용값 확인 + 헤더 렌더 육안 확인.

## 페르소나 진입 화면 통일 — 인트로 영상 → 소개 모달 (2026-07-29, 사장 지시)

**문제**: 같은 페르소나인데 **어디서 들어왔느냐에 따라 다른 경험**을 줬다.

| 경로 | 전 | 문제 |
|------|-----|------|
| 친구초대 링크 `?p=` | 소개 모달(얼굴·이름·소개문·**기능칩**·CTA) | — |
| 메인에서 카드 클릭 | 인트로 영상/이미지만 → 바로 채팅 | **뭘 물어봐야 할지 모른다** |

코드 주석에 *"친구 링크로 온 사람은 채팅창만 보고는 뭘 물어봐야 할지 모른다"*고 적혀 있었는데,
**메인으로 온 사람도 똑같다.** 정작 메인 유입이 더 많은데 정보를 덜 받고 있었다.

**조치**:
- `showPersonaGuide(persona)` 공통 함수 추출 — 전엔 딥링크 처리부에만 인라인으로 있어
  두 경로가 갈렸다. 이제 **같은 함수**를 쓴다(기능칩 구성·명부 안내·소개문 로직 공유).
- `handlePersonaClick`의 인트로 분기를 소개 모달로 교체(사장 결정: "인트로 없이 가보자").
  ★`introVideoUrl` 자산은 DB에 그대로 있으므로 되살리려면 **이 분기만** 바꾸면 된다.
- ★**비회원 경로(`handleGuestPersonaClick`)는 인트로 유지** — 가입 유도라는 별개 목적이 있다.
  일괄 교체했으면 그 흐름까지 망가졌다(고칠 때 **잃는 것**을 확인한 사례).

**의존성 주의**: `showPersonaGuide`를 딥링크 `useEffect`에서 쓰므로 의존성 배열에 추가했다
(빠뜨리면 낡은 클로저를 참조한다).

## 페르소나 진입 후속 수정 2건 — 전수조사 결과 (2026-07-29, 사장 지적)

인트로를 소개 모달로 바꾼 뒤 **두 가지가 깨져 있었다**. 둘 다 "고칠 때 잃은 것을 확인하지
않은" 같은 계열의 실수다(같은 날 캡처 크롭에서 유나 이미지를 잃은 것과 동일).

**① "OO와 시작하기"를 눌렀는데 기능 보드가 뜬다**
- 원인: CTA가 `features.length === 1`이면 그 기능을 자동 실행했다 —
  **"카드 1개 = 기능 링크"라고 가정**한 것.
- ★전수조사: **담당 기능이 하나뿐인 페르소나가 9명**이라 전부 걸렸다 —
  신은비(명품감정)·유나(타로)·서아(뉴스)·윤채원(주식)·설아(스윙)·정우진(보험)·
  강지훈(전자책)·향기(웹툰)·박하진(홈페이지). (2개 이상: 지우2·이아린4·윤채린4·도결8)
- 수정: `autoRunFeatureKey` 필드 신설 — **기능 링크(?f=)로 왔을 때만** 채운다.
  페르소나 링크·메인 카드 클릭은 비어 있어 CTA가 채팅으로 간다.
  ★검증은 **양쪽 다** 했다: 페르소나 링크→채팅 ✅ / 기능 링크→보드(기존 동작 유지) ✅.

**② "시작하기를 눌러도 인사를 안 한다"**
- 원인: 인트로 영상을 없애면서 **`prefetchOnly` 호출도 함께 사라졌다.** 원래는 인트로가
  재생되는 동안 백그라운드로 세션 생성+greet를 끝냈는데, 그 시간이 통째로 없어진 것.
- 수정: 딥링크(?p=) 경로와 **같은 순서**로 맞췄다 —
  `handleSelectPersona(id, {prefetchOnly:true})` → `handleSelectPersona(id)`.
  `usePersonaSession`의 `loadingRef` 가드가 2차 호출의 세션 생성을 스킵해 중복이 없다
  (prefetchOnly는 `setActivePersonaId`를 건드리지 않으므로 화면 전환은 2차가 담당).
- 검증: 운영에서 CTA 클릭 후 **인사말 1.2초**에 등장 확인.

## 재방문 인사 + 채팅방 진입 자동 스크롤 (2026-07-29, 사장 지시)

**① 재방문 인사** — "연속으로 다시 들어가니 인사가 안 나온다"
- 원인은 버그가 아니라 **서버의 의도된 가드**였다: `sessions.ts /greet`가 마지막 메시지로부터
  **2시간 이내면 `skipped: true`**로 통째로 건너뛴다. 방금 얘기하다 들어왔는데 또 첫인사를
  하면 어색하고 AI 비용도 매번 나가기 때문(인사말은 재사용하지 않고 매번 새로 생성한다).
- 다만 그 결과 재입장 시 **아무 반응 없이 예전 대화만** 덩그러니 보였다.
- ★조치는 **세 번 바뀌었다**: 2시간 → 10분 → (1분 시도) → **"사용자가 실제로 대화했는가"**.
  10분으로 줄여도 "연속으로 들어가는데 안 나온다"(실제 재입장 간격은 1~2분)였고,
  1분으로 더 줄이려 하자 **사장이 어뷰징 위험을 지적**했다 — 인사말은 AI 호출인데 포인트가
  나가지 않으므로 나갔다 들어오기만 반복하면 비용이 무한정 나간다.
  → **시간 간격만으로는 못 푼다**(짧으면 어뷰징, 길면 인사를 안 함). 기준을 바꿨다:
  **직전 인사 이후 사용자가 한 마디라도 했는가**
    · 인사받고 아무 말 없이 나갔다 재입장 → skip (이미 인사했으니까)
    · 대화한 뒤 나갔다 재입장          → 인사 (새 방문이니까)
  반복 입장으로는 인사가 절대 늘지 않아 어뷰징이 원천 차단되고, 그래서 시간 가드를
  1분으로 짧게 둬도 안전하다. ★판정은 AI 호출이 아니라 **DB 조회 2번** — 걸러지면 Gemini
  호출 자체를 안 하므로 총비용은 오히려 줄어든다(사장 질문 "비용 2배 아니냐"에 대한 답).
- ★**role 실측**: `user`=사용자 / **`model`=AI 일반 대화 응답** / **`assistant`=인사말**.
  'assistant'만 세는 건 의도적 — model까지 세면 대화 직후 재입장에서 항상 skip된다.
- 재방문 프롬프트를 **"한 문장으로만, 지난 대화를 요약·설명하지 말고 인사만"**으로 조였다.
  ★프롬프트 분기(`isFirstVisit ? 첫인사 : 재방문`)는 **이미 있었고 가드가 막고 있었을 뿐**이다.
- 검증(운영): 25분 경과 상황을 만들어 재입장 → *"다시 찾아주셔서 반갑습니다, 당신의 별이 또
  이곳을 밝혀주었네요."* 한 문장 생성 확인. 테스트로 바꾼 DB 시각은 원복.

**② 채팅방 진입 자동 스크롤** — "매번 손으로 내리기 귀찮다"
- 원인: 스크롤 `useEffect`의 의존성이 `[currentSession.messages]`뿐이라 **배열 참조가 바뀔
  때만** 돈다. 이미 로드된 세션으로 재진입하면 참조가 그대로여서 스크롤이 안 걸렸다.
- 조치: `scrollMessagesToBottom(instant?)`로 추출하고, **`activePersonaId` 변경**을 따로 잡아
  진입 시 1회 스크롤.
  - ★`behavior:'auto'`(instant) — 진입하자마자 주르륵 흐르면 산만하고, 메시지가 많으면
    애니메이션이 끝나기 전에 사용자 스크롤과 충돌한다. 새 메시지 도착 시에는 기존대로 smooth.
  - ★`requestAnimationFrame` 2회 후 실행 — 렌더·레이아웃이 확정돼야 `scrollHeight`가 정확하다.
  - 기존 컨테이너 탐색 로직은 유지(`scrollIntoView`는 모바일에서 상단 헤더까지 밀어낸다).
- ★한 번만 시도하면 놓친다(사장 "스크롤은 무조건 잘 넘겨야 한다"): 세션 로드가 비동기라
  진입 직후엔 메시지가 비어 있고, 이미지·폰트가 늦게 뜨면 높이가 다시 늘어난다.
  → **바닥에 닿을 때까지 120ms 간격 재시도**(최대 2초). 이미 바닥이면 즉시 멈춰 사용자
  스크롤을 방해하지 않고, 언마운트 시 rAF·timer를 정리한다.
- 검증(운영): 유나·신은비·도결·윤채린 **4개 페르소나 전부 바닥에서 0px** 확인.

## 🎴 페르소나 진입 시트 — 화면 전환 없이 덮기 (2026-07-31, `6a8c58a`)

사장 지적 "메인에서 채팅 페이지로 전환되는 게 불만이다. 큰 카드처럼 떠서 그 페르소나에만
집중되게 해달라."

**원인은 카드 크기가 아니라 구조였다.** 소개 JSX가 `chat` return **안에만** 있어서, 띄우려면
`goTo('chat')`으로 화면을 통째로 갈아치우는 게 선행돼야 했다. 지난주 덧댄 땜질도 전부 여기서
파생된 증상이었다:
  ① 모달 뒤에 채팅이 이미 렌더돼 배경 클릭이 "닫기"가 아니라 채팅 진입처럼 보임
  ② ✕가 `goTo('main')`으로 메인을 다시 그려 보던 스크롤·탭이 날아감

- **`components/PersonaEntrySheet.tsx` 신설** (App.tsx 인라인 192줄 → 4줄).
  `main`·`chat` **양쪽 return이 같은 `personaEntrySheet`를 렌더**한다 → 전환 자체가 불필요.
  ①②는 원인이 사라지므로 **배경 클릭 닫기를 되살리고**, ✕는 레이어만 닫는다(화면 보존).
- **레이아웃(C안)**: 모바일=전체 시트 / 데스크톱=중앙 카드(400px). 데스크톱까지 전체화면으로
  덮는 건 과하다 — 넓으면 카드로도 충분히 집중된다. 얼굴 96→**128px**, 대화 안내를 기능칩
  **위로**(대화가 주·기능이 곁가지), CTA 하단 고정(스크롤 끝까지 안 내려도 보임).
  근거: 모바일에 데스크톱식 중앙 박스를 쓰지 말 것(LogRocket) / 독립 과업은 전체화면(Medium) /
  모달은 "막혔다" 느끼면 내용과 무관하게 즉시 닫힘(NN/g).
- **진입 경로 6곳에서 `goTo('chat')` 제거** — 전환은 시트 CTA에서 **딱 한 번**.
  (?p= 딥링크 / ?f= 딥링크(보드형·채팅형) / 메인 페르소나 카드 / 메인 기능 카드(타로·운세))
  ★기능 카드로 들어온 경우 `runFeatureKey`를 시트에 넘겨 CTA가 채팅 대신 그 기능을 연다
  ("유나와 시작하기"). 안 넘기면 타로를 눌러도 맨 채팅에 떨어진다.
- ★**실측으로 발견한 실사용 버그**: `RewardAlertModal`(z-9999)이 시트(z-85)를 통째로 덮어
  **CTA가 눌리지 않았다.** 초대 링크(`?ref=`)로 처음 온 사람이 정확히 이 경우 — 환영 알럿과
  소개가 동시에 뜬다. → 알럿이 떠 있는 동안 시트를 렌더하지 않는다(닫으면 나타남).
  ※이때 "리렌더 때문에 검사기가 실패한 것"으로 한 번 오진했다. `document.elementsFromPoint()`로
  클릭 지점의 z-index 스택을 직접 찍고서야 잡았다 — **"검사기 문제"로 결론내기 전에 실제 겹침을
  확인할 것**. Playwright의 "intercepts pointer events"는 대부분 진짜 겹침이다.
- 검증(운영): 메인 페르소나 카드·메인 기능 카드(타로)·`?p=`·`?f=hair` **전 경로에서 전환 없음**,
  ✕ 닫기 시 메인 유지, CTA에서만 전환. 런타임 에러 0건.

---

## 쇼츠 UI 3건 — 폼 스크롤 / 카테고리 카드 접기 / 음성 선택 (2026-08-02, 사장 지적)

### ① 폼이 중간부터 보이던 문제 (`b3f51cc`)

사장 지적 *"첫 포커스가 탑이어야 하는데 중간 이하로 맞춰져 있는 것 같다"*.
쇼츠 만들기 폼에 들어가면 상단 입력칸이 아니라 "분위기·언어 / 참고 자료" 근처가 첫 화면으로 보였다.

**원인 — 증상은 "스크롤"이었지만 범인은 "포커스"였다.**
intro의 `✨ 내 쇼츠 만들기 시작`과 form의 `💎 100P — 시나리오 5개 받기` 버튼이
**className도 부모 내 위치도 같아 React가 같은 DOM 노드로 재사용**한다. intro에서 클릭해
포커스를 얻은 그 노드가 form에서도 포커스를 쥔 채 폼 맨 아래로 이동하고, 브라우저가
"포커스된 요소는 보여야 한다"는 규칙에 따라 거기까지 스크롤을 밀었다.

실측(운영 512×592): `scrollTop 0 → 1128`(전체 1749). 이벤트 순서도 `scroll(1128)` → `focusin(BUTTON)`.

**수정** — `step` 변경 시 `activeElement.blur()` + `scrollRef.scrollTo({top:0})`.
`blur()`까지 넣은 이유: 스크롤만 되돌리면 포커스가 남아 다음 상호작용(탭 키 등)에서 또 밀린다.
폼뿐 아니라 **모든 단계 전환**에 적용돼 같은 증상이 다른 단계에서 재발하지 않는다.
검증: 배포 후 `scrollTop: 0` 실측 확인.

### ② 카테고리 카드 접기 (`79ad97b`)

사장 지적 *"카테고리가 계속 늘어나니 복잡해 보인다"*(생일축하 추가 예정).
카드마다 5줄(이름·요약·입력안내·예시·완성대본)을 항상 펼쳐두니 **4개만 돼도 한 화면을 넘겼다.**

- 접힌 상태: **아이콘 + 이름 + 한 줄 요약 + 사진 배지**
- 상세는 **호버(데스크톱)·탭(모바일) 둘 다**로 펼침. 선택된 카드는 계속 펼쳐둠
- 실측: 생일축하 포함 **6개가 스크롤 없이 한 화면**에 들어옴

**★사장 제안은 "마우스 오버 시 모달"이었으나 탭까지 받도록 넓혔다.**
폰에는 호버가 없어 **호버 전용으로 만들면 모바일에서 그 정보에 영영 닿을 수 없다.**
그런데 그 정보(사진 필요 여부·완성 대본)야말로 카테고리를 고르는 근거다.

**★"사진 필요/불필요"는 접지 않고 배지로 남겼다** — 설명이 아니라 **선택 조건**이다.
사진이 없는 회원은 이걸 보고 카테고리를 거른다.

### ③ 어드민 음성 선택 패널 (`cb72aae`)

사장 요청 *"어드민 쇼츠 메뉴에 만들어놔라, 내가 들어보고 선택해서 저장하게"*.
`ShortsVoicePanel.tsx` — 성별별 후보 + ▶미리듣기 + 저장. 상세는 `doc/features/shorts_maker.md`.

**★"나이별"은 넣지 않고 화면에 이유를 명시했다** — Google TTS에 나이 파라미터가 없어
"20대/40대" 라벨을 붙이면 실제 결과와 어긋나는 거짓 표기가 된다.
화면 문구: *"나이는 지정할 수 없어요(TTS에 해당 기능 없음) — 목소리마다 인상이 다를 뿐이라
직접 들어보고 고르시면 됩니다."*

### 부수 — 보관함 중복 복사 확인창 (`3613b3d`)

복사 여부가 화면 state에만 있어 **새로고침하면 기억이 날아가고** 이미 보관한 영상도 복사 버튼이
멀쩡히 활성화됐다(중복이 쌓인 경로 중 하나). 마운트 시 보관함 목록을 출처 id로 역인덱싱해
"✅ 보관됨" 초기 표시. 서버 409 수신 시 "이미 보관함에 있어요(제목·날짜). 그래도 복사할까요?"
확인 후 승인 시에만 `force`로 재복사.

★`apiService.request()`도 함께 수정 — 예전엔 `new Error(data.error)`로 **message만 남기고 본문을
버려서**, 409가 실어 보내는 "어떤 항목과 중복인지"를 호출측이 읽을 수 없었다. 이제 `err.status`·`err.body` 부착.

---

## 2026-08-06 — 어드민 화면 문구가 만든 오해 4건 (AI 스튜디오·회원관리)

이날 받은 지적 6건 중 **4건은 기능이 정상인데 화면 때문에 생긴 오해**였다.
UI 문서에 남기는 이유는, 넷 다 **코드가 아니라 문구·라벨을 고쳐서 풀렸기** 때문이다.

### ① 버튼이 활성인데 눌러도 반응이 없으면 "고장"으로 보인다

`doGenerate` 첫 줄이 `if (!prompt.trim()) return` 이라 **프롬프트가 비면 조용히 반환**됐다.
버튼은 멀쩡히 눌리는데 아무 일도 안 일어나니 "생성하기가 안 눌린다"가 된다.
→ `disabled={busy || !prompt.trim()}` + 문구를 **`⌨ 먼저 프롬프트를 입력하세요`** 로.
★**막을 거면 버튼을 잠그고, 왜 잠겼는지 버튼 자신이 말하게 한다.**

### ② placeholder 는 라벨이 아니다

프롬프트·네거티브 **텍스트창 2개가 소제목 없이 나란히** 있어, 아래 칸이 무엇인지
placeholder 를 읽어야만 알 수 있었다. ★**placeholder 는 글자를 넣는 순간 사라져
정작 입력 중엔 아무 안내가 없다.**
→ `✏️ 그리고 싶은 것(필수)` / `🚫 빼고 싶은 것 (네거티브)(선택)`.
★전문용어(`네거티브`)는 **뜻을 제목에 같이** 넣는다 — 낱말만으론 반대 의미인 걸 알 수 없다.

### ③ 설명은 '무엇인가'가 아니라 '언제 쓰는가'

모델 설명이 `실사 인물 최고 — 프로필·헤드샷` 처럼 **성격**만 적혀 있어
"설명이 더 헷갈려"라는 지적을 받았다. 읽는 사람이 알고 싶은 건 *그래서 뭘 골라야 하나*다.
→ `★사람 사진은 이걸로` / `⛔이걸로는 그림이 안 나옴`. 기호는 **★=권장 / ⛔=금지 둘만**.
★**`<option>` 안에서는 줄바꿈·색상이 브라우저에서 무시**되고 닫힌 `select` 는 폭이 좁아
괄호 설명이 **잘린다** → 설명은 드롭다운 **바깥**에 다시 띄운다(★초록 / ⛔빨강).

### ④ 목록에서 숨겼으면 '숨겼다는 사실'을 남긴다

체험계정(`provider='guest'`)이 정회원과 섞여 쌓여 "정리가 안 된다"고 보였다
(실제로는 7일 크론이 정상 작동 중 — `referral_system.md` 참조).
→ 기본 숨김 + `체험계정 보기` 체크박스. 단:
- ★머리말에 **`· 체험 N명 숨김` 상시 표시** — 숫자가 없으면 이번엔 "다 사라졌다"는 **반대 오해**
- ★**검색 중에는 숨긴 것도 보여준다** — 찾는데 안 나오면 "없다"고 결론 내린다
- 펼쳤을 때 구분되도록 '체험' 배지 / `provider` 없는 옛 응답은 **정회원 취급**(배포 과도기 안전)

### ⑤ 선택지가 여럿인데 고르는 UI 가 없으면, 코드가 무엇을 고르는지 확인할 것

확대 모델이 2종 설치됐는데 코드는 `upscalers[0]`(**목록 첫 번째**)를 썼다. 정렬상 우연히
좋은 쪽이 먼저였을 뿐, 새 모델을 받아 순서가 바뀌면 **더 나쁜 쪽으로 조용히 바뀐다**(에러도 없다).
→ 이름으로 고르게 하고(`pickUpscaler`), 우열이 분명한 선택지는 **칸을 만들기보다
무엇이 쓰이는지 화면에 밝히는** 편이 낫다 — `좋은 쪽(4x-UltraSharp)으로 자동 적용됩니다`.

관련 커밋: `52cf660` `f42aaed` `87b3f7c` `7dc3042` `37bdcb1`

---

## 2026-09-09 — 랜딩에서 기능을 눌렀는데 "창만 닫힌다" (사장 지적, `3e6353c`)

설아 진입 시트에서 **스윙 분석**을 누르면 시트만 사라지고 아무것도 안 떴다.

### 원인 — 값이 아니라 **렌더 위치**

`onFeature` 는 ①`setDeepLinkGuide(null)`(랜딩 닫기) → ②`FEATURE_ACTIONS[key]()` 순으로 돈다.
그런데 스윙 UI 가 App.tsx **chat return 안에만** 있었다
(1724줄 `if (screen === 'main')` 얼리리턴 뒤). 랜딩은 main 에서 열리므로
**상태만 켜지고 화면엔 아무것도 안 뜬다** → 사용자 눈에는 "창만 닫힌" 것.

★★**tsc·빌드·번들 grep 이 전부 통과한다.** 코드는 있고 **있는 자리**가 틀린 문제라
정적 검사로는 잡히지 않는다. 유나 타로(`onFeature` vs `onStart`)와 **같은 계열**이다.

### 전수 감사 — 21개 중 골프 3개만 빠져 있었다

| 상태 | 기능 |
|---|---|
| main·chat 양쪽 ✅ | news·stock·hair·outfit·golf-course 등 **18개** |
| chat 전용 🔴 | `swing` · `golf-swing` · `golf-record` |

`golf-course` 는 09-08 에 추가하며 제대로 넣었는데 스윙 쪽이 누락돼 있었다.

### 수정

- 스윙 입력 모달·분석 보드를 **main 블록에도** 추가
  (`ErrorBoundary` + `closeBoardAndReturn` — 닫으면 랜딩 복귀)
- 진행 상태 카드(50줄)는 **복붙 대신 `swingProgressCard` 변수로 추출**해 양쪽에서 사용.
  복붙하면 나중에 **한쪽만 고치는** 사고가 난다.

### 회귀 테스트 — 개별 키가 아니라 **목록 전체**를 본다

`entryReturn.test.ts` 가 `featureBoardOpeners` 를 파싱해 **모든 보드가 main 에도
렌더되는지 자동 검사**한다. 새 기능을 붙일 때 또 빠뜨릴 자리라 이렇게 짰다.
★수정을 되돌려 **실제로 3건이 실패하는지 확인**한 뒤 원복했다.

### 🔴진단 중 오판 2회 — 기록해 둔다

- **기능 키를 DB 만 보고 단정했다.** DB `Persona.features` 가 `["golf-swing",…]` 라
  그 키인 줄 알았는데 **진입 시트의 실제 키는 `swing`** 이었다.
  시트 features 는 `showPersonaGuide()` 가 **`FEATURES_GRID`**(MainPageNew.tsx)에서 만들고,
  DB 쪽은 **채팅 안 기능 버튼**용이다 — **경로가 다르다.**
  React fiber 의 `key` 를 실측해서야 확정됐다.
- **배포 후 "아직 안 고쳐졌다"고 오보했다.** 클릭 1초 뒤 `innerText` 만 보고 판정했는데,
  모달은 뜨는 데 시간이 걸리고 **배경이 메인 화면**이라 "메인으로 돌아갔다"로 읽혔다.
  **스크린샷을 여니 모달이 정상 동작 중**이었다 → 누른 뒤 **무엇이 보이는지 눈으로** 볼 것.

## 모바일에서 저절로 움직이는 연출 — `<video>` 를 쓰지 말 것 (2026-09-17)

aigov 히어로를 실사풍 진입 영상으로 만들었는데 **사장님 폰에서만 늘 정지**였다.
원인은 코드가 아니라 **브라우저 정책**이었다.

```
NotAllowedError: play() failed because the user didn't interact with the document
SamsungBrowser/30.0 · 버퍼 '충분' · 데이터절약 꺼짐 · 모션줄임 꺼짐
```

`autoplay` · `muted` · `playsinline` · `preload="auto"` 를 **다 갖춰도** 막힌다.
크롬보다 엄격하고, **어떤 사이트도 우회할 수 없다**(유튜브도 마찬가지다).

→ **애니메이션 WebP 로 만든다.** 이미지는 자동재생 정책을 받지 않아 삼성 인터넷·크롬·
사파리 어디서나 저절로 움직인다. 실측: 터치 없이 화면 변화 **95.9%**.

```sh
ffmpeg -i in.webm -vf "fps=12,scale=800:-1:flags=lanczos" \
       -loop 1 -q:v 58 -compression_level 6 out.webp
#      ^^^^^^^ 0=무한 반복, 1=한 번만 재생하고 마지막 프레임에서 멈춘다
```

실측 용량(3.38초 원본 기준): 800×450/12fps **699KB** · 960×540/15fps 1,127KB.
히어로 배경이고 위에 어두운 스크림이 깔리므로 800×450 으로 충분하다.

### 함께 걸린 함정

- **`object-fit:cover` + 높이가 변하는 박스 = 그림이 다시 잘린다.** 히어로가
  300px→130px 로 접히자 사용자에게 **"이미지 크기가 바뀐 것"** 으로 보였다.
  `object-position` 으로는 못 고친다(50%/42%/30% 전부 ~60 으로 동일) —
  **높이를 바꾸지 않는 것**이 답이다(실측 130px=61.9 · 260px=40.4 · **300px=27.0**).
- **`<img>` 는 요청이 중단되면 `load`·`error` 가 둘 다 안 온다**(실측).
  이벤트만 믿지 말고 **타임아웃 상한**을 둘 것.
- **모션 감소 대응은 `defer` 스크립트로는 늦다.** 애니메이션이 이미 돌아버린다 →
  파싱 시점의 인라인 스크립트에서 정지 이미지로 바꿔 끼운다.

### 진단이 먼저다

같은 증상에 원인을 **네 번** 잘못 짚었다(캐시 → 포스터 타협 → 터치 수용 → autoplay 속성).
**Chromium 에뮬레이션에서는 정상 재생됐기 때문이다.** 돌파구는 폰이 자기 상태를 직접
보고하는 **임시 진단 페이지**였고 한 번에 확정됐다(확인 후 삭제).
→ **내 환경에서 되는 것은 사용자 환경의 증거가 아니다. 추측을 두 번 이상 쌓기 전에
진단 도구부터 만들 것.**

### 정적 파일은 "넣는 것"과 "서빙되는 것"이 다르다 (2026-09-17)

aigov 히어로 자산을 `public/` 에만 넣었더니 **Vercel 운영은 멀쩡한데 로컬 Node 서버는 404**
였다. `src/app.js` 가 **경로 허용목록**(`paths`)으로 3개만 서빙하기 때문이다. Vercel 은
`public/` 을 직접 주므로 운영에서만 우연히 동작했고, 그 탓에 browser-smoke 가 계속 실패해
**ASTRA 개발 단계가 `execution_paused` 로 멈췄다.**

등록할 곳이 **세 층**이다:
1. `public/` 에 파일
2. `src/app.js` 의 `paths` + `types`(MIME)
3. `test/browser-smoke.mjs` 의 `ALLOWED_PATHS`

하나라도 빠지면 404 이거나 "예상 밖 요청이 있습니다"로 막힌다.
★**운영이 되는 것은 로컬이 된다는 증거가 아니다**(역도 성립) — 배포 경로가 다르면
같은 파일도 다르게 취급된다.

### 인라인 `<script>` 는 CSP 를 어긴다 — 운영이 CSP 를 안 보내면 몰래 통과한다

aigov 서버는 `content-security-policy: script-src 'self'` 를 보내는데, **운영(Vercel 정적)은
CSP 헤더를 안 보낸다.** 그래서 내가 넣은 인라인 스크립트가 **운영에서만 돌고 앱 서버에서는
막혔다**(browser-smoke 가 잡았다). → 로직은 외부 `.js` 로 옮기고, 파싱 시점 처리가 필요하면
그 파일 맨 앞에서 한다.

### 브라우저 테스트는 첫 실패에서 멈춘다

browser-smoke 가 320 에서 실패했을 때 "375·720 은 통과"로 읽고 기준을 세웠다가 틀렸다 —
**뒤쪽 조합은 실행조차 되지 않았다.** 조합별 결과를 말하려면 **그 조합이 실제로 돌았는지**
먼저 확인할 것.
