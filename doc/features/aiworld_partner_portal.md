...
- ★**프런트·백엔드 양쪽을 함께 고쳐야 한다** — 한쪽만 고치면 화면 또는 서버에서 막힌다.
  - `shared-api/routes/aimp/partner-auth.ts` — 서버 검증 + 오류 문구("4~72자")
  - `ai_mp/sites/ainara-cube/index.html` — 암호·암호확인 `minlength`, placeholder("4자 이상")
- 무차별 대입 완화는 기존 `authLimiter`(`express-rate-limit`, 10분 10회)가 담당한다.
- 검증은 **운영 API 실호출**로 했다: 5자 → 비번 통과(다음 검증인 성명 오류로 진행),
  3자 → `"비밀번호는 4~72자로 입력해주세요."`. 운영 HTML 에 옛 값 잔존 0건.
- ⚠️보안 메모: 파트너 계정은 정산·수익 정보에 접근한다. 4자는 약한 편이므로
  여유가 생기면 2단계 인증·로그인 알림을 검토할 것(사장께 언급, 지시는 없었음).
- 커밋: shared-api `988498c` · ai_mp `a4b9ccc`.
