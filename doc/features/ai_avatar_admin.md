...
`ai-3d-avatar/deploy/server3/ai-avatar-worker.service`(커밋 `5dbbcc5`, `systemd-analyze verify`
통과). **서버3 미기동 상태로 만들어 뒀고, 다음에 서버를 켤 때 설치한다**(설치는 과금 무관).

🔴**운영 순서 — 작업을 먼저 넣고 서버를 켠다.** 서버3은 큐가 비면 5분 크론이 유휴를 세다
30분에 끈다(실측). 큐에 QUEUED/RUNNING 이 있으면 스탬프가 갱신돼 유지된다.

**남은 것**: 서버3을 다음에 켤 때 유닛 설치. 자동 기동은 실행 이력이 더 쌓인 뒤 재검토.
