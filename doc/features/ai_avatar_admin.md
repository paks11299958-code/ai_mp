...
### 2026-09-02 S3FD 교체 완료 — 라이선스 부채 해소 (GPU 17분, 약 ₩360)

위에서 남겨둔 S3FD 를 이어서 처리했다. **이제 idle·립싱크 양쪽 모두 상용 가능하다.**
정본 사본·설치 절차: `ai-3d-avatar/deploy/musetalk/`(커밋 `e97b125`, `e0ee442`).

**구조를 읽어보니 InsightFace 와 같은 모양이었다.**
립싱크 품질을 만드는 것은 **DWPose**(Apache-2.0)의 68점 랜드마크이고, S3FD 는
`(x1,y1,x2,y2)` 정수 4개짜리 박스 하나만 돌려준다. 그 값은 ①얼굴 유무 ②랜드마크 박스가
이상할 때의 폴백 에만 쓰인다. 사용처도 `preprocessing.py` 3줄뿐이었다.

★★**반환 계약이 이 교체의 핵심이다.** 배치 길이·순서가 어긋나거나 못 찾았을 때 `None`
대신 0박스를 주면 **에러 없이 입이 밀린 영상**이 나온다 — 코드로는 안 보이고 영상을
봐야 안다. 그래서 회귀 16개가 길이·순서·`None` 을 집중적으로 못박고, 사보타주 2종으로
실제 실패를 확인했다. (길이 검사를 먼저 하도록 고쳤다 — 그전엔 `IndexError` 로 죽어
진짜 원인이 트레이스백에 묻혔다.)

**검증**: 검출기 직접 비교 123프레임 — 양쪽 모두 미검출 0, IoU 평균 0.800,
좌표 평균차 8.9px(윗변 차이가 큼: MediaPipe 는 랜드마크 껍데기라 이마를 덜 포함).
립싱크 A/B 전체 평균차 0.08%·입영역 0.72%, **입 모양 육안 비교에서 사장 확인 완료**.
🔴**결정적 검증**: `s3fd.pth` 를 숨긴 뒤에도 정상 생성되고 B판과 픽셀 동일(최대차 0.18/255).
삭제 후 재생성해도 다시 내려받지 않는다 → **89MB 영구 삭제**.

★**형제도 함께 고쳤다**: `scripts/preprocess.py:45` 에도 `FaceAlignment` 생성이 있었다.
...
### 2026-09-03 Phase 4 — 검수 점수표와 게시 게이트 (GPU 불필요, 비용 0)

🔴★★**Phase 2 에 구멍이 있었다.** 게시 가드는 `stage === 'REVIEW'` 를 보는데
**stage 를 REVIEW 로 올리는 경로가 어디에도 없었다**(실측: stage 를 쓰는 곳은 게시 시
`'PUBLISHED'` 하나뿐). 모든 프로젝트가 `REFERENCE` 에 묶여 **게시가 항상 409** 였다.
Phase 4 가 그 문을 여는 부분이다.

**점수표**: 정체성·시간축·립싱크를 1~5로 매긴다. 합격선은 **정체성 4 · 나머지 3**.
★정체성이 가장 높은 이유는 3D 실험을 접었던 근거와 같다 — 얼굴이 그 사람으로 안 보이면
나머지가 아무리 좋아도 못 쓴다.
★**점수 미입력은 통과가 아니다** — 안 매긴 것을 통과로 치면 검수 없이 게시된다.
★**`PUBLISHED` 는 재검수 실패로 강등하지 않는다** — 내리는 것은 rollback 의 일이고,
낮추면 운영 중 게시물의 롤백 이력이 꼬인다.

- `shared-api/routes/aimp/aiAvatarReview.ts` — 판정 순수 함수 (커밋 `864fd25`)
- `POST /projects/:id/review` — 점수를 **행으로 쌓고**(덮어쓰지 않음) stage 를 전이
- Phase 4: ✅점수표·승인·게시·롤백·운영 화면 실클릭 검증 완료(2026-09-03). 립싱크 실행 배관은 남음
  ★CHECK 제약으로 1~5 를 **DB 에서도** 막는다. 앱 검증만 믿으면 다른 경로로 6점이 들어가고
  그때는 게이트가 아니라 데이터가 거짓말을 한다.
- 화면: `AiAvatarPanel.tsx` 검수 점수표 (커밋 `20c58b8`)
  ★통과 판정과 사유는 **서버 응답을 그대로** 보여준다. 화면에서 미리 계산하면 합격선이
  두 곳이 되어 서버만 고쳤을 때 화면이 옛 기준으로 거짓말을 한다.

**검증**: 서버 회귀 14개(사보타주 3종 실패 확인) · 패널 16개 · admin 52개 · check · build.
운영 DB 실측 15건(잔존 0). **운영 사이트 실클릭**: 390/1440 양쪽에서 점수표 표시,
정체성 3점 → "합격선 미달"·"게시 잠김", 5점 → "검수 통과"·"게시 가능", pageerror 0.

🟡**어드민 탭바 가로 넘침(1505px)은 기존 문제다** — AI 아바타 탭에 들어가기 전
기본 탭에서도 1684px 이 난다(내 변경과 무관, 별건).

### 2026-09-03 Phase 5 — 실행 배관과 게시 안내 (서버3 미기동·GPU 비용 0)

**① 워커** `ai-3d-avatar/deploy/server3/ai_avatar_worker.py` (커밋 `29bca1c`, **서버3 미설치**)
`AiAvatarJob` 을 폴링해 LivePortrait·MuseTalk 를 실제로 돌린다. 기존 `gpu_worker.py` 와
같은 패턴(원자적 claim `FOR UPDATE SKIP LOCKED` · stale 회수 · 실패 격리)이다.

- ★**별도 워커**다. `GpuJob` 워커는 ComfyUI 를 부르지만 아바타는 micromamba 환경 두 개에서
  직접 실행한다. 섞으면 한쪽 장애가 다른 쪽을 멈춘다.
- ★**heartbeat 를 일부러 안 쓴다.** `idle_shutdown.sh` 가 DB 큐를 보고 **스스로** 스탬프를
  갱신하므로(09-02 수정) 워커가 또 쓰면 두 곳이 같은 파일을 다툰다.
- ★**타임아웃은 서버2 디스패처와 같아야 한다** — 어긋나면 한쪽은 '도는 중', 다른 쪽은
  '죽었다'로 봐서 **같은 작업이 두 번 돌고 GPU 요금이 두 배**가 된다(회귀로 고정).
- ★**산출물이 없으면 성공으로 넘기지 않는다** — READY 인데 재생할 게 없는 상태가 가장 찾기 어렵다.
- ★미지원 종류(`BUILD_REVIEW`)는 `FAILED` 로 드러낸다. QUEUED 로 되돌리면 영원히 도는 작업이
  되고 그때마다 서버가 켜진다.

**② 게시 반영 안내** (커밋 `dd75c29` · 화면 `b6d2649`)
두 사이트 모두 영상을 **git 저장소에서 정적으로** 서빙한다. **자동 커밋·푸시는 하지 않기로
했다(사장 결정)** — 어드민 버튼 하나가 운영 사이트를 즉시 바꾸면 잘못 눌렀을 때 되돌릴
시간이 없다. 대신 게시 응답에 `applied:false` 와 대상별 저장소 경로·체크섬을 담고,
화면이 **"원장에만 기록됨 — 사이트는 아직 그대로입니다"** 를 띄운다.
★"게시했습니다"로 끝내면 사람은 사이트가 바뀐 줄 안다 — 그게 이 변경이 막는 것이다.
...
### 2026-09-03 Phase 5 실행 검증 — **전 구간 실동작 확인** (GPU 약 40분)

워커를 서버3에 설치하고 **실제로 큐를 물려 돌렸다.** 계획대로 GPU 를 안 쓰는 작업으로
배관부터 확인한 뒤 렌더로 넘어갔다.

| 작업 | 결과 | 소요 | 산출물 |
|---|---|---|---|
| `PREPARE_REFERENCE` | READY | 0초 | 입력 확인(GPU 미사용, 배관 검증용) |
| `GENERATE_IDLE` | READY | **29.9초** | 148프레임 1024x1024, 평균밝기 231 |
| `GENERATE_LIPSYNC` | READY | **89.5초** | 122프레임 1024x1024, 평균밝기 231 |
| `BUILD_REVIEW` | **FAILED / UNSUPPORTED_KIND** | — | 의도대로 드러남(무한루프 아님) |

★**MediaPipe 교체가 실제 실행에서도 유지된다** — 립싱크를 돌린 뒤에도
`~/.cache/torch/hub/checkpoints/` 가 **0개**다(S3FD 재다운로드 없음).
★산출물은 파일 존재가 아니라 **프레임 수·해상도·평균밝기까지** 확인했다(검은 화면 방지).
회수본: `~/seoa_2d_avatar/phase5_verify/`.
...
#### 같은 날 후반 — Wan 90도 인사로 교체 · 답변 시 영상 유지

**답변 중에도 대기 영상을 쓴다**(사장 지시, `99b9eb9`). `seoa-speaking-poc.mp4` 는
특정 문장으로 만든 립싱크 샘플이라 AI 가 무슨 답을 하든 **같은 입 모양**이 돈다 —
내용과 어긋나 어색하다. ★SPEAKING 상태와 배지 문구는 남겼다(실시간 립싱크를 붙일 자리).

**인사 영상을 Wan 2.2 결과로 교체했다**(`6947d8e`, 최종 `9dff2ab`).
LivePortrait 는 고개 각도 한계가 5.3도지만 Wan i2v 는 **상체가 굽어 정수리가 보인다.**
인사 5.04초 + 멘트 7.42초 = **12.46초**, 자막은 전부 **+5.04초** 밀었다(인사 구간 무음).
★704x1216 세로형 → 1024 정사각형은 **블러 패딩**으로 넣는다(같은 영상을 흐리게 깔아 배경).
크롭은 인물 크기가 튀어 이음새가 드러나고, 흰 여백은 파란 띠가 보인다.

🔴🔴**깨진 파일을 코드로 네 번 고친 실패** — 이 항목이 이 문서에서 가장 값진 기록이다.
교체 후 상담창이 `fallback` 에 빠지고 자막이 안 나왔다. canplay 대기 / 전역 error 가드 /
로딩대기 error 가드를 차례로 고쳤지만 전부 헛발이었고, 네 번째에 **파일 무결성 검사**로
원인이 나왔다:
```
[aac] Input buffer exhausted before END element found   (반복)
video = 12.44초  /  audio = 6.80초   ← 오디오가 절반에서 끊김
```
`concat -c copy` 가 오디오 스트림을 잇지 못해 **파일 자체가 깨져 있었다.**
→ concat 필터로 재인코딩(`setsar=1` 필요). ★**"코드를 고쳤는데 증상이 그대로"면
입력 데이터를 의심할 것** — `ffmpeg -v error -i <파일> -f null -` 한 줄이면 처음에 끝났다.

**회귀 24개**(`sites/ainara-cube/tests/ai-consult-modal.test.mjs`), 운영 실클릭 7건 통과.
⚠️`sites/ai-companion` 은 아직 옛 3D(glb)를 쓰는 **다른 사이트**다. aiworld = `ainara-cube`.

### 2026-09-03 운영 방식 결정 — **수동 기동으로 간다**

`AVATAR_DISPATCH_ENABLED` 는 **꺼진 채로 둔다**(사장 결정). 서버를 켜는 것은 사람이 정하고,
켜진 뒤 작업 처리는 워커가 자동으로 한다.

★**이유**: 아바타 작업은 사람이 어드민에서 만드는 것이라 자동 기동이 벌어주는 게 거의 없다
(하루에 몇 번씩 도는 성격이 아니고, 작업 넣고 켜는 데 1분이면 된다). 반면 자동 기동은
**버그로 작업이 반복 생성되면 아무도 안 보는 사이 계속 켜진다.** 하루 상한 6회는 최악을
막는 장치지 새는 것을 막는 장치가 아니다. 실행 이력이 아직 4건뿐이라 타임아웃 회수·복구가
여러 번 돌아본 뒤에 얹는 것이 순서다.

★**단, systemd 유닛은 분리해서 넣는다** — 유닛은 과금과 무관하고, 오히려 사람이 손으로
워커를 띄우는 방식이 더 위험하다(SSH 를 끊으면 워커가 죽어 작업이 RUNNING 에 박제되고,
부분 유니크 인덱스 때문에 같은 작업을 다시 만들 수도 없다).
`ai-3d-avatar/deploy/server3/ai-avatar-worker.service` — ✅**설치·검증 완료**(커밋 `578d419`).
`active`/`enabled` 확인에 그치지 않고 **유닛이 띄운 워커가 사람 개입 없이 작업을 처리하는 것**
까지 확인했다(큐 적재 → 10초 내 `READY`). `Restart=always` 도 `kill -9` 로 실증(PID 1441→1568).

🔴**운영 순서 — 작업을 먼저 넣고 서버를 켠다.** 서버3은 큐가 비면 5분 크론이 유휴를 세다
30분에 끈다(실측). 큐에 QUEUED/RUNNING 이 있으면 스탬프가 갱신돼 유지된다.

**남은 것**: 자동 기동(`AVATAR_DISPATCH_ENABLED`)은 실행 이력이 더 쌓인 뒤 재검토.
그 외 배관은 전부 끝났다 — 작업을 큐에 넣고 서버3을 켜면 워커가 자동으로 처리한다.
★서버2 `AVATAR_DISPATCH_ENABLED` 는 **아직 설정하지 않았다**(기본 `0`) — 큐가 쌓여도
GPU 는 켜지지 않는다. 화면 배너도 이 상태 그대로 "자동기동 OFF"로 적었다.
