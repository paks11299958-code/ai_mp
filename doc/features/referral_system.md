...
## 🚨 전환 0명 진단 → 비회원 체험 온보딩 신설 (2026-08-07)

사장 지적("회원가입이 저조하다")에서 출발해 실측으로 병목을 특정하고, 빠진 단계를 채운 작업.
**진단이 세 번 뒤집혔다** — 감이 아니라 DB를 봐야 했던 사례로 남긴다.

### 1) 어드민 숫자가 상황을 가리고 있었다

| 월 | 어드민 표시 | 실제 정회원 | 체험계정 |
|---|---|---|---|
| 2026-04 | 6 | 6 | 0 |
| 2026-05 | 20 | 20 | 0 |
| 2026-06 | 16 | 16 | 0 |
...
7월 43명은 반등처럼 보였지만 **실회원은 이미 꺾이는 중**이었다(주별 7/06 11 → 7/13 4 → 7/20 2 → 0).
**7/26 이후 정회원 가입 0명**이 3주간 아무에게도 안 보였다.
→ `DailyBizReport.guestUsers` 분리(위 db_schema 참조) + 경영리포트 화면에 '신규(정회원)'·'체험' 열 분리.

### 2) 기각된 가설 2개 — 돈 문제가 아니었다

- ~~"500P가 부족해 못 쓴다"~~ → **기각**. 정회원 1,000P·게스트 500P이고 주력기능은 200P라 2~3회 가능.
  그런데 **게스트 43명 중 42명이 0회 사용**, 잔액 500P를 그대로 남기고 이탈했다.
  (★조사 중 "헤어스타일 577P" 오독이 있었다 — 총액/건수 평균이었고, 1,000P 단가 시절 41건이
  섞여 있었다. 평시 단가는 `MenuLimit` 실측으로 **200P**.)
- ~~"첫 1회 무료를 만들면 된다"~~ → **기각**. `GUEST_FREE_TRIAL_DESC` 상수만 보고 "이미 있다"고
  판단했다가, 주석을 읽어보니 **2026-07-28 폐지**돼 있었다(상수는 과거 집계 조회용으로만 잔존).
...
| 체험 가입(500P 지급) | 43 | — |
| 채팅 세션 생성 | 21 | 49% |
| 메시지 1건 이상 | 21 | 100% (총 28건 = 1인당 **1.3건**) |
| **유료 기능 사용** | **1** | **5%** |
| 정회원 전환 | **0** | **0%** |

절반은 화면까지 왔고 한 마디 던지고 나갔다. **채팅은 무료**(`DAILY_FREE_CHAT_LIMIT=100`)라
가장 먼저 만나는 것이 가장 차별점 없는 기능이었다.

### 4) 원인 — 비회원에게는 500P의 존재조차 안 알렸다

게스트 자동생성은 **`?ref=` 유입에만** 걸려 있었다. 그냥 방문한 사람이 기능을 누르면
`App.tsx`가 곧바로 `setShowAuthModal(true)` — **무엇을 하는 곳인지, 무엇을 주는지 한 줄도 없이**
가입만 요구했다.

| 유입 경로 | 기능 클릭 시 (변경 전) |
|---|---|
| `?ref=` 링크 | 게스트 자동생성 → 500P → 바로 사용 |
| **그냥 방문** | **가입 창(설명 없음)** |

### 5) 조치 — `GuestTrialModal` 신설

단계적 가입 유도의 **빠진 첫 단계**를 채웠다:
```
비회원 → [체험회원+500P] → 2~3회 사용 → 잔액 소진 → 정회원
          ↑ 통째로 비어 있던 구간      ↑ 이미 있음   ↑ 이미 있음
```
- 클릭한 기능의 `name`/`catch`/`desc`/`palette.accent`(`FEATURES_GRID`)를 그대로 주입 →
  **기능이 늘어도 모달은 손댈 필요 없다**.
- 체험 시작 후 `pendingDeepLink`로 **누르려던 기능까지 이어서** 보낸다.
- ★게스트 계정은 **버튼 클릭 시에만** 생성(렌더 시점 생성 금지) + 연타 가드.
  표시만으로 만들면 한 사람이 계정을 여러 개 받는다(230→231→232 전례, 위 §버그 참조).
- 로그인 경로는 하단 "이미 회원이신가요? 로그인"으로 유지.

**초대 링크 기본 목적지**도 `home` → `hair`로 변경(`InviteFriendModal`). 딥링크 인프라(`?f=`)는
이미 있었고 **기본값만 홈이라 안 쓰이고 있었다**.

### 6) 지식iN 채널 재평가 — 게시 여부 회수 장치

`ref=KIN` 방문이 `ReferralVisit`에 0건이라 "게시가 안 된 것"으로 오판했으나,
**가입 기록은 `ChannelSignup`에 있었다**(KIN 63명 / YOUTUBE 5 / SHORTS_TAROT 2).
다만 63명 중 **7/28 하루에 49명**이 몰린 허수이고, **사장 확인 결과 평시 유입은 하루 2~3명**.

초안 54건이 전부 `status='pending'`이라 "안 올려서"인지 "올렸는데 클릭이 없어서"인지
판별 자체가 불가능했다 → 텔레그램 승인 요청에 **[✅게시완료 / 🚫건너뜀] 버튼** 부착
(`kin_answer_worker.resolve()` → `posted`/`skipped`, 멱등).
★`getUpdates` 소비는 **`telegram_listener` 한 곳 원칙** 유지 — 워커는 버튼만 붙이고
콜백 라우팅은 리스너(`approve|reject:KIN-` → `handle_kin_callback`)가 처리한다.

**답변별 세분 코드(`ref=KIN_<id>`)는 착수했다가 되돌렸다** — 하루 2~3명을 쪼개도 결정 근거가
안 된다는 사장 판단("오버 아니냐")이 맞았다. `CHANNEL_PREFIXES`에 `KIN_`을 넣는 변경도 함께 롤백.

### 커밋·검증

| 저장소 | 커밋 |
|---|---|
| ai_mp | `5b3dd82` 경영리포트 분리 + 초대링크 목적지 / `927199e` GuestTrialModal |
| rag | `52c4165` biz_report 분리집계 / `10c83b2` 지식인 게시완료 버튼 |
| shared-api | `cbf1e5c` daily-reports에 guestUsers 추가 (서버1 자동배포 완료) |

- 검증: `npm run check`·`tsc`·빌드 통과 + **운영(aichat.dbzone.kr)에서 Playwright로
  비회원 꿈해몽 클릭 → 모달 노출 실측** + `npm run smoke` 통과.
- ★**번들 해시로 배포 여부를 판단하지 말 것**(2회 반복한 오판). 로컬 빌드 해시와 Vercel
  빌드 해시는 원래 다르다 — 이번에도 "미배포"라 보고했으나 실제로는 배포 완료 상태였고,
  **새 코드는 옛 해시 파일 안에 들어 있었다**. 검증은 *이번 변경에만 있는 문자열 grep* +
  *운영 URL 실제 동작*으로 한다.
- 파일: `ai_mp/frontend/components/GuestTrialModal.tsx`(신설)·`App.tsx`·
  `components/InviteFriendModal.tsx`·`components/admin/BizReportPanel.tsx`,
  `rag/biz_report.py`·`kin_answer_worker.py`·`telegram_listener.py`,
  `shared-api/routes/aimp/biz.ts`.

### 다음에 볼 지표

체험회원 수는 늘어날 것이고 **그건 성과가 아니라 중간 단계**다.
어드민 경영리포트의 **'신규(정회원)' 열이 0에서 움직이는지**가 진짜 지표다.
지식인은 게시완료 버튼이 며칠 쌓이면 "게시 대비 유입"을 처음으로 숫자로 볼 수 있다.

## 체험회원 재발급 차단과 만료 전환 (2026-09-26)

사장님 점검으로 체험회원이 사실상 반복 발급될 수 있는 구조를 확인했다. 서버 제한은 IP별
`10분 5회`뿐이어서 로그아웃하거나 7일 뒤 게스트 계정이 정리되면 같은 브라우저에서도 새 계정과
1,000P를 다시 받을 수 있었다.

- `shared-api/routes/aimp/auth.ts`: 첫 체험 발급 시 1년 HttpOnly·Secure 쿠키
  `aimp_guest_trial_used`를 함께 설정한다. 이 쿠키가 있는 `/guest-register` 요청은 새 계정을 만들지
  않고 `409 / GUEST_TRIAL_USED`와 회원가입 안내를 반환한다. 변경 전에 발급된 게스트도 `/me`에서
  쿠키를 보완한다.
- `frontend/App.tsx`: 체험 로그인 성공이나 기존 게스트 복원 시 브라우저 사용 이력을 남긴다.
  로그아웃 후 기능을 다시 누르면 체험 지급 모달을 재노출하지 않는다.
- `frontend/components/GuestTrialModal.tsx`: 사용 이력이 있거나 서버가 409를 반환하면
  `체험이 만료되었습니다`와 `무료 회원가입`을 보여주고 회원가입 탭으로 바로 연결한다.
- 공용 IP를 영구 차단하면 회사·카페 이용자를 같이 막으므로 사용하지 않았다. 쿠키와 저장소를 모두
  삭제하거나 다른 기기를 사용한 동일인까지 막으려면 휴대전화·이메일 인증이 필요하다.

커밋: shared-api `d481c69`, ai_mp `816238d`; Vercel 업로드 제외 설정 `c135a31`.
운영 배포 `dpl_iPBgihfpnsqsCZuLpt8hcU7BW4PP`. 운영 API에 체험 쿠키를 넣은 요청은 409였고,
