...
## 2026-08-06 — 어드민 회원 목록에서 체험계정 기본 숨김

**발단**: 사장 지적 — "체험회원 왜 정리가 안 된 거야? 아직도 어드민 회원메뉴에 보임".

### ★먼저 DB로 확인했더니 삭제는 정상 작동 중이었다
...
- 서버1 크론 `0 21 * * * .../internal-cron/cleanup` **등록 확인**
- `~/aimp-cron.log` 에 `cleanup FAIL` **0건** — 실패 없이 돌고 있었다
- 가장 오래된 것도 7일차(07-30 00:12)로 방금 경계에 닿은 건 → 다음 회차에 삭제됨

즉 7일 정책은 **모든 체험회원을 지우는 게 아니라 7일 지난 것만** 지운다.
화면에 보이던 대부분은 **아직 기한이 안 된 정상 보유분**이었다.
★어드민 화면의 이상해 보이는 수치를 **버그로 단정하기 전에 DB로 확인**할 것
 — 2026-07-27 '체험회원이 초대가입?' 오해와 같은 유형이다.

### 그래서 삭제 주기가 아니라 '화면'을 고쳤다

보관 기간을 줄이면 **체험 중인 사람의 계정이 사라지므로** 정책은 손대지 않는다.
유입이 있는 날엔 수십 건이 한꺼번에 쌓여 정회원이 묻히는 게 실제 불편이므로,
화면에서 접는 쪽으로 푼다.

- 회원 목록에서 **체험계정(`provider='guest'`) 기본 숨김** + `체험계정 보기` 체크박스
- ★머리말에 `· 체험 N명 숨김`을 **항상 표시** — 숫자가 안 보이면 이번엔
  "체험회원이 사라졌다"는 **반대 오해**가 생긴다
- ★**검색 중에는 체험계정도 검색된다** — 특정 체험회원을 찾는데 안 나오면 "없다"고 오해한다
- 펼쳤을 때 어느 줄이 체험인지 보이도록 **'체험' 배지**(amber)
- `provider` 없는 옛 응답은 **정회원 취급** — 배포 과도기에 실수로 감추지 않도록

### 서버 변경

`GET /api/aimp/admin/users` 응답에 **`provider` 추가**. 이게 없어서 프론트가
체험계정을 구분할 수단이 아예 없었다.

**실측 6종 통과**: 기본접힘 / 보기ON / 검색시노출 / 검색+ON / 정회원검색 / 옛응답.
★번들 생존 확인 — `c=e=>e.provider===\`guest\`,l=e.filter(c).length`.

### 커밋

| 저장소 | 커밋 |
|---|---|
| shared-api | `786c02f` 회원 목록 응답에 provider 추가 (서버1 자동배포 완료) |
| ai_mp | `f42aaed` 체험계정 기본 숨김 + 배지·건수 |

- 파일: `shared-api/routes/aimp/admin.ts`, `ai_mp/frontend/components/admin/UsersPanel.tsx`,
  `ai_mp/frontend/services/apiService.ts`
