...
> 갱신: 2026-06-21 | 상태: **✅ 구현·배포 완료** (아래 "기획" 본문은 초기안 히스토리)

## ★최종 사양 (2026-06-21)
- **컨셉**: 내 사진 + 현재 나이 → **미래(노화) 모습**. 과거(회춘)는 제거(성인→10대 골격 어색+수요↓).
- **목표 나이(사장 지정 표, 10단위 룩업 futureTargetAges)**: 10대 30·50·80 / 20·30대 40·60·80 / 40대 50·70·80 / 50대 60·70·80 / 60대 70·80·90 / 70대 80·85·90 / 80대+ 85·90·95. (현재보다 미래만, 95 상한)
- **선택형 + 개당 100pt**: 나이대별 3개 중 원하는 것만 선택(최대 3) → 고른 것만 생성. 개당 100pt(MenuLimit `agetransform`, 1~3개=100~300, 어드민 조정). 실비 ~53원/장이라 흑자.
- **Before/After 슬라이더**: 결과는 원본(지금)↔미래 드래그 슬라이더로 겹쳐보기 + 탭 전환.
- **생성 신호등(헤어와 공유)**: 같은 나노바나나 쿼터 → `lib/imageGenBusy.ts` 공용(429시 90초). `GET /age-transform/status`·`/hair/status` 둘 다 사용. 프론트 15초 폴링→🟢/🔴, 혼잡 시 버튼 비활성(헛 대기 방지).
- **저장/취소**: 생성분 미저장 → 저장 눌러야 DB확정+개당×개수 차감, 취소=미저장(GCS만, gcs-cleanup). 차감은 결과물 저장 시점.
- **백엔드** `routes/aimp/age-transform.ts`: generate(selectedAges 표값만 허용=위변조 방어)/save(개당×개수 차감)/list/status. **AI** `generateAgeTransform`(lib/gemini.ts). **프론트** `AgeTransformBoard.tsx`. DB `AgeTransform`(imagesJson={"42":url,...}). 상세 메모리 [[project_age_transform]].
