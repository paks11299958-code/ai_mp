...
PartnerAccount:
  ...
  approvalRole(PARTNER|APPROVER|ADMIN), lastLoginAt, createdAt, updatedAt

PartnerApplication:
  id(BIGSERIAL), partnerAccountId(unique FK), referrer(nullable)
  status(PENDING|CONTACTED|APPROVED|REJECTED)
  referrerAccountId(nullable FK), privacyVersion, privacyAgreedAt, managerMemo(nullable)
  approvedByPartnerAccountId(nullable FK), approvedByAdminUserId(nullable)
...
`PartnerApprovalHistory`: applicationId, actorType(ADMIN|PARTNER), actorId, fromStatus,
toStatus, createdAt. 관리자·지정 승인자의 상태 변경 감사 이력이다.
