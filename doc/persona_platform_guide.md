...
### ★노출 제어 2축 — `adminOnly` 와 `isVisible` 은 뜻이 다르다 (2026-08-18 정리)

혼동해서 한쪽만 거르는 사고가 실제로 났다. **목록을 조회하는 코드는 반드시 둘 다** 본다.

| 필드 | 뜻 | 일반 회원 | 어드민 |
|---|---|---|---|
| `adminOnly: true` | 관리자 전용 기능 | 안 보임 | 보임 |
| `isVisible: false` | **숨김(데이터 보존)** — 운영에서 잠시 내림 | 안 보임 | **보임**(다시 켤 수 있어야 함) |

- **어드민 조회는 둘 다 거르지 않는다.** 어드민에서까지 숨기면 **되살릴 방법이 없어진다.**
- **숨김의 범위는 "목록 비노출"이지 "기능 차단"이 아니다**(2026-08-18 사장 확인).
  딥링크(`/chat?persona=…`)로 직접 들어오는 경로는 열어 둔다 — 이미 링크를 가진 회원은 계속 쓴다.
- ★**목록만 막고 랭킹을 빠뜨리면 반쪽이다.** 숨긴 페르소나가 인기 랭킹에 뜨면 숨긴 게 아니다.
  현재 적용 지점: `routes/aimp/personas.ts` **목록(GET /)** 과 **랭킹(GET /ranking)** 둘 다.
- ★**2026-08-18 이전에는 `isVisible` 이 조회에서 통째로 무시**되고 있었다 — 저장·프론트는
  멀쩡한데 **읽는 쪽만 빠져** 어드민에서 체크를 꺼도 화면이 그대로였다(`AI 학습코칭` 실측).
  **"필드가 있다"≠"동작한다"** — 새 노출 플래그를 만들면 **조회 지점 전수**를 같이 확인할 것.
