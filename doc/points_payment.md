...
**★전 기능 확대 완료(2026-08-11, `shared-api 61ccd8a`)** — 4곳 → **34곳(17파일)**.
- **catch형 15곳**: `e.message === 'INSUFFICIENT_POINTS'` 분기. 형태가 동일하고 `e`에 값이
  실려 있어 그대로 전달.
- **사전검사형 15곳**: 잔액을 직접 비교하는 지점. 조건식·변수명이 제각각이라 **파일별로**
  잔액을 변수에 담아 계산해 넘겼다(age-transform은 단가×장수, ebook은 항목별 단가라
  `required`가 곱셈인 곳들이 있다).
- `checkMenuAccessOrRespond` 자체도 헬퍼를 쓰도록 통일. `EDIT_FEATURE[kind]` 같은 **동적
  featureKey**도 그대로 전달. `featureLabels.ts` 28개와 대조해 **누락 0** 확인.
- ★**정규식 일괄치환을 쓰지 않았다**(08-08에 그 방식으로 파일 2개가 깨진 이력). (파일, 정확한
  원본, 대체)를 명시하고 **원본이 정확히 1회일 때만** 교체, 하나라도 안 맞으면 전체 중단.
  같은 파일에 동일 문자열이 여러 번인 곳(homepage·shorts-maker)은 **뒤따르는
  `deductMenuPoints` 설명 문자열까지 포함**해 유일하게 만들었다.
- 검증: `tsc --noEmit` 통과(중간에 **import 누락 5건을 tsc가 잡아** 보강) / 관상·손금은
  포인트 0 임시유저로 **실제 HTTP 402 본문 확인** / 헬퍼 직접 호출로 쇼츠·홈페이지·전자책 확인 /
  ★**값이 없을 때도 `undefined`가 안 나가는 것** 확인. 임시유저는 검증 후 삭제.
...
## 결제 완료 알림 (2026-07-29 신설)

결제가 성사되면 **사장님에게 문자(SMS) + 텔레그램**으로 즉시 통보한다.

- 구현: `shared-api/lib/adminNotify.ts` — `notifyPayment({username, amount, points})`
  호출 지점은 `routes/aimp/payments.ts`의 포인트 지급 성공 직후.
- 문자는 기존 SOLAPI `lib/sms.ts`(인증문자용) 재사용, 텔레그램은 신규.
  **둘을 독립 실행**(`Promise.allSettled`)해 하나가 죽어도 다른 하나는 간다.
- ★**알림 실패가 결제를 깨뜨리면 안 된다**: fire-and-forget + 내부 전량 try/catch.
  결제는 이미 성사됐는데 문자 실패로 500을 주면 회원이 결제 안 된 줄 안다.
- ★**서버1에서 텔레그램 fetch가 ETIMEDOUT**: `api.telegram.org`가 IPv6로만 resolve되는데
  그 경로가 막혀 있다(`curl -4`는 정상). Node `fetch`는 IP 버전을 못 고르므로
  `https.request`에 **`family: 4`**를 줘 IPv4를 강제한다.
- 환경변수(서버1 `.env`): `ADMIN_NOTIFY_PHONE` · `TELEGRAM_BOT_TOKEN` · `TELEGRAM_CHAT_ID`.
  미설정 시 해당 경로만 조용히 생략(운영 중 값만 넣으면 바로 켜짐).
- SMS는 **90바이트 초과 시 LMS로 요금이 오르므로** 본문을 짧게 유지.

## 환불 처리 (2026-07-29 문서화)

현재 환불은 **수동 처리**다(결제 총 2건·환불 0건 규모라 자동화는 과잉이고,
환불은 되돌릴 수 없어 사람 판단이 필요). 절차는 **어드민 → 💸 환불 절차** 탭에 상설화:

1. 요청 확인(닉네임·결제일시·사유) 2. 가능 여부 판단(약관 제5조) 3. 토스에서 결제 취소
4. 포인트 회수 요청 5. 회원 안내

- ★**토스 취소와 포인트 회수는 한 세트** — 하나만 하면 돈은 돌려주고 포인트는 남아 정산이 어긋난다.
- ★환불액은 **잔여 비율로 환산**한다. 패키지에 보너스가 섞여 있어(10,000원=**11,000P**)
  사용분을 원 단위로 빼면 틀린다. 탭에 계산기 내장.
- 수수료 10%는 약관상 "부과될 수 **있습니다**"=재량 → 소액에서 500원 떼면 반발이 크므로
  초기 사례는 면제 권장(계산기 기본값도 면제).
