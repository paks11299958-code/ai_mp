...
- **첫 기능 1회 무료**: `deductMenuPoints`(차감 단일 지점)에서 `provider='guest'`이고 미사용이면 차감 없이 통과시키고 **0원 MENU 거래(`체험 첫 1회 무료`)**를 남긴다 → 이 거래 존재 여부가 멱등 판정. 두 번째 호출부터 정상 차감. 어뷰징은 `guest-register`의 IP 제한이 1차 방어.
