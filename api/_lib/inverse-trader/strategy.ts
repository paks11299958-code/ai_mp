 ...
 * 국내 **ETF/ETN** 호가가격단위 표.
 *
 * ★2026-08-20 실측으로 정정: 이전 주석은 "ETF는 가격대와 무관하게 5원 단일"이었으나
 *   **2,000원 미만에서는 틀리다.** KODEX 200선물인버스2X(252670) 최근 10일 종가가
 *   78 / 88 / 79 / 76 / 81 / 87 / 94 / 96 / 97 / 96 원으로 **하나도 5의 배수가 아니다.**
 *   5원 단일로 두면 저가 ETF에서 주문가가 격자에 안 맞아 전부 거부된다.
 *
 * ★고가 구간은 원래 주석이 맞았다 — ETF는 주식 밴드를 따르지 않는다.
 *   KODEX 레버리지(122630)는 10만원대인데 종가가 110,465 / 113,525 로 **5원 단위**다
 *   (주식이라면 10만원대는 100원 단위여야 한다). 그래서 주식 밴드를 그대로 쓰면 안 되고,
 *   2,000원 경계 하나만 두는 것이 실측과 맞는다.
 *
 * ★가격대가 낮을수록 1틱의 비중이 커진다는 점이 전략에 결정적이다:
 *   5,000원짜리의 1틱(5원) = 0.10% 인데, 78원짜리의 1틱(1원) = **1.28%** 다.
 *   같은 "1호가 먹기"라도 저가 종목에서는 스캘핑이 아니라 변동성 베팅이 된다.
 */
export const ETF_TICK_BANDS: TickBand[] = [
    { under: 2_000, tick: 1 },
    { under: null, tick: 5 },
];
