...
import { adminApi, type DevProjectFileRow } from '../../services/apiService';
...
export const HomepageMakerPanel: React.FC = () => {
    ...
    const [uploading,setUploading]=useState(false), [refImages,setRefImages]=useState<DevProjectFileRow[]>([]);
    ...
    const reset=()=>{setBrief(EMPTY_HOMEPAGE_MAKER_BRIEF);setUseReview(true);setSaved(null);setRefImages([]);setMessage('');setError('');};
    ...
    const saveSpec=async()=>{
        ...
        try{
            ...
            else{const d=await adminApi.createDevProject(body);setSaved({id:d.project.id,signature});setRefImages(d.project.files?.filter(f=>f.kind==='image')??[]);}
    ...
    const pickImages=async(files:FileList|null)=>{
        ...
        try{
            const room=Math.max(0,20-refImages.length);
            for(const f of Array.from(files).slice(0,room)){
                if(f.size>5*1024*1024){setError(`'${f.name}'은 5MB를 넘어 건너뜁니다.`);continue;}
                const dataUrl=await new Promise<string>((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result));r.onerror=()=>reject(new Error('파일을 읽지 못했습니다.'));r.readAsDataURL(f);});
                const d=await adminApi.uploadDevImage(saved.id,dataUrl);
                setRefImages(v=>[...v,d.file]);
            }
            if(files.length>room)setError(`참조 이미지는 최대 20장입니다. ${files.length-room}장은 올리지 않았습니다.`);
            setMessage('참조 이미지를 올렸습니다. 개발 명세에 함께 전달됩니다.');
        }catch(e:any){setError(e?.message||'이미지 업로드에 실패했습니다.');}
        finally{setUploading(false);if(fileRef.current)fileRef.current.value='';}
    };
    const removeImage=async(fileId:number)=>{
        try{setUploading(true);setError('');await adminApi.deleteDevImage(fileId);setRefImages(v=>v.filter(f=>f.id!==fileId));}
        catch(e:any){setError(e?.message||'이미지 삭제에 실패했습니다.');}finally{setUploading(false);}
    };
    ...
    return <div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
            ...
            <section className="rounded-2xl border border-gray-800 bg-gray-900/70 p-4 sm:p-5 space-y-3"><div><h3 className="text-sm font-bold text-white">참고 이미지</h3><p className="mt-1 text-xs text-gray-400">로고·현재 화면·원하는 분위기를 개발AI가 직접 확인합니다.</p></div><input ref={fileRef} aria-label="참고 이미지 파일" type="file" multiple accept="image/png,image/jpeg,image/webp,image/gif" className="hidden" onChange={e=>void pickImages(e.target.files)}/><button type="button" onClick={()=>fileRef.current?.click()} disabled={!saved||dirty||busy||uploading||refImages.length>=20} className="w-full rounded-xl border border-gray-700 bg-gray-950/60 px-4 py-3 text-sm font-bold text-gray-300 disabled:opacity-35">{uploading?'올리는 중…':saved&&!dirty?'＋ 참고 이미지 선택':'명세 저장 후 이미지 추가'}</button>{refImages.length>0&&<div className="grid grid-cols-3 gap-2 sm:grid-cols-4">{refImages.map(f=><div key={f.id} className="group relative overflow-hidden rounded-lg border border-gray-800 bg-gray-950"><a href={f.url} target="_blank" rel="noreferrer"><img src={f.url} alt={f.fileName} className="h-20 w-full object-cover"/></a><button type="button" aria-label={`${f.fileName} 삭제`} onClick={()=>void removeImage(f.id)} disabled={uploading} className="absolute right-1 top-1 h-6 w-6 rounded bg-black/75 text-xs text-white hover:bg-red-800">×</button></div>)}</div>}<p className="text-[11px] text-gray-500">PNG·JPG·WEBP·GIF · 장당 5MB 이하 · {refImages.length}/20장</p></section>
