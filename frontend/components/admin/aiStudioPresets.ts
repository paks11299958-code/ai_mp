 ...
 *
 * ★문구 규칙(2026-08-05 사장 지적 — "설명이 헷갈린다"):
 *   기존엔 '실사 인물 최고', '보정 전용' 같은 **성격**만 적었더니, 그래서 내가
 *   뭘 골라야 하는지가 안 보였다. 그래서 **용도(언제 쓰는지)를 앞**에 두고,
 *   고르면 안 되는 것은 ⛔로 못박는다. 목록 순서도 추천 → 비추천 순으로 둔다.
 *   ★`★`=권장 / `⛔`=고르지 말 것. 이 두 기호만 알면 되게 유지한다.
 */
const MODEL_NOTES: Record<string, string> = {
    ...
    //   · flux1-schnell-fp8(16.1GB) — "사람 사진은 RealVis가 나았다"로 이미 판정됨
    //   · sd_xl_refiner_1.0(5.7GB)  — 단독으로는 그림이 안 나오는 보정 전용
    //   · flux1-dev-fp8             — 비상업 라이선스라 애초에 미설치
    //   다시 필요하면 어드민 [모델 관리]에서 받을 수 있다(카탈로그에는 남아 있음).
    //   ★단 FLUX 계열은 IPAdapter FaceID가 SDXL 전용이라 함께 못 쓴다 —
    //     Flux+FaceID를 하려면 PuLID-Flux 등 별도 구축이 필요하다.
    // ★확대 후보정 2종 — 2026-08-06 같은 시드로 **소재를 바꿔가며** 두 차례 A/B 실측.
    //   ⑴사진(인물·제품 = 질감이 살아야 하는 것) → **UltraSharp 우위**
    //     인물: 속눈썹·잔머리가 한 올씩 살고 모공까지 / RealESRGAN 은 피부를 뭉개 플라스틱처럼.
    //     제품: 광택 하이라이트·가장자리가 살아남 / RealESRGAN 은 반사광을 뭉개 밋밋해짐.
    //   ⑵플랫 일러스트(면·선 위주) → **RealESRGAN 우위**
    //     색 면이 깨끗하고 경계가 매끄럽다. UltraSharp 는 평면에 **자글자글한 노이즈**를 만든다.
    //   ★즉 같은 "선명하게 하는 힘"이 사진에선 디테일로, 평면 그림에선 노이즈로 나타난다.
    //     ★"인물에서 졌으니 나쁜 모델"이 아니라 **소재가 갈린다** — 1차 결론을 2차에서 뒤집었다.
    //   ★속도: 1차(인물)엔 둘 다 10초였는데 2차 재측정은 UltraSharp 60초 / RealESRGAN 10초.
    '4x-UltraSharp':           '★사진(인물·제품)은 이걸로 — 머리카락·광택이 또렷해집니다(기본)',
    RealESRGAN_x4:             '일러스트·평면 그림에 좋음 — 사진에 쓰면 질감이 뭉개집니다',
};
