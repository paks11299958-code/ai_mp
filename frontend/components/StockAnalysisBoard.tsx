...
export const StockAnalysisBoard: React.FC<Props> = ({ onClose, onConsult }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[60px] md:pt-[84px] md:px-6 md:pb-6" style={{ background: 'rgba(45,37,32,0.55)', fontFamily: "'Pretendard Variable', Pretendard, -apple-system, system-ui, sans-serif" }}>
            <div className="w-full max-w-5xl h-[calc(100vh-60px)] md:h-auto md:max-h-[calc(100vh-108px)] rounded-t-[20px] md:rounded-[20px] flex flex-col overflow-hidden" style={{ background: T.bg, border: `1px solid ${T.border}`, boxShadow: '0 24px 60px rgba(45,37,32,0.2)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '12px 18px', borderBottom: `1px solid ${T.border}`, background: T.surface, flexShrink: 0 }}>
                    ...
                    <div className="flex items-center gap-1" style={{ flexShrink: 0 }}>
                        <HelpButton
                            ...
                            accent={T.accent}
                            steps={[
                                { title: '종목명 입력', desc: '분석할 종목명(예: 삼성전자)을 입력하고 + 를 눌러요.' },
                                { title: 'AI가 분석', desc: 'Gemini·Claude·GPT 3개 AI가 함께 분석해요(몇 분 걸려요).' },
                                { title: '결과 확인', desc: '투자의견·점수와 상세 보고서를 보고, 다운로드도 할 수 있어요.' },
                            ]}
