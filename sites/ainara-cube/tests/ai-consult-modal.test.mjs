...
test('★자막 시각이 실제 음성과 맞는다', async () => {
  ...
  // ★2026-09-03: 앞에 Wan 인사 동작 5.04초가 붙어 전체 12.46초가 됐다.
  //   넘어가면 자막이 영상보다 오래 떠 있다.
  assert.ok(cues.at(-1)[1] <= 12.5, `마지막 자막이 영상 길이를 넘는다: ${cues.at(-1)[1]}`);
  // ★인사 동작 구간(0~5.04초)은 무음이라 자막이 없어야 한다.
  //   여기에 자막이 뜨면 소리도 없는데 글자만 나온다.
  assert.ok(cues[0][0] >= 5.0, `인사 동작 중에 자막이 뜬다: ${cues[0][0]}`);
  ...
  // ★실측한 문장 사이 쉼(음성 기준 3.255~3.792 = 영상 기준 +5.04)에
  //   세 번째 자막이 걸쳐 있으면 안 된다.
  assert.ok(cues[2][0] >= 8.6, `두 번째 문장 시작이 너무 이르다: ${cues[2][0]}`);
});
