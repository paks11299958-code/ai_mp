...
> **Opus에게:** 새 시안 생성 시 이 문서 하단의 **선택 이력 테이블**을 반드시 확인하세요.
> 사용자가 과거에 어떤 스타일을 선택했는지 파악해서 취향에 맞는 시안을 제작하세요.
> 예: 쇼핑몰에서 "따뜻한 자연스러운" 스타일을 여러 번 선택했다면 → v1도 그 계열로 제안
...
> 새 시안 생성 시 이 테이블을 먼저 확인하세요. 사용자가 과거에 선택한 스타일 경향을 파악해
> 취향에 맞는 v1 시안을 제안합니다. 시안 확정(사용자 선택) 시마다 한 행씩 추가하세요.

| 날짜 | 프로젝트 | 사이트 유형 | 선택 시안 | 선택된 스타일 특징 | 비고 |
|------|---------|-----------|---------|-----------------|------|
| (아직 없음) | | | | | |
...
## 관련 문서
- [[architecture|ai_mp 아키텍처]] — sites/ 폴더 구조
- [[dev_index|Dev Agent 규칙]] — 작업 규칙
- [[react-typescript|React+TypeScript]] — 기존 서비스 패턴
