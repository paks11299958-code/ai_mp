...
800으로 올림. Haiku 4.5 출력 단가($5/1M)에서 월 100회 기준 원가 증가는 (800-500)×100=
30,000토큰×$5/1M=**$0.015(약 21원)** 수준으로 무시 가능. (사용자 제시 "약 40원"은 200→320
증분 가정과 유사한 오더 — 아래 원가 재계산은 실측 반영해 320토큰 근사치로 갱신.)
...
| 6.4 질문하기(입력400·출력320, 100회) | **Haiku 4.5** | $0.280 | **$0.196** |
| 6.5 주간 리포트 | Sonnet 5(유지) | $0.018 | $0.0184 (변동 없음) |
| **합계** | | **약 $0.64(약 900원)** | **약 $0.56(약 782원)** |

- **약 13% 절감(118원)**. PRD 2,000원 기준 대비 여유가 45%→약 61%로 개선(5차 시점의
  703원 추정보다는 78원 늘었으나, `max_tokens` 상향으로 절단 재발 위험을 없앤 대가로
  타당한 트레이드오프).
- 6.4 상한(하루10회) 완전 소진+12주 장기사용 최악 케이스는 약 1,697원 수준. 6.2/6.3
  실측 전까지는 여전히 가정치 기반.
