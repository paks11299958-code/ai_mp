...
## ★비공개 저장소지만, 적기 전에 확인할 것

`paks11299958-code/ai_mp`는 **2026-08-22에 Public → Private으로 전환**했다.
지금은 외부에서 이슈를 볼 수 없다.

★**다만 방심하지 말 것.** 전환 전 약 4개월(2026-04-22~08-22, 커밋 1,330개)간
저장소가 공개돼 있었다. 그 기간에 공개된 내용은 **비공개로 바꿔도 사라지지 않는다**
(포크·클론·캐시가 남아 있을 수 있다). 아래 원칙은 계속 지킨다.
